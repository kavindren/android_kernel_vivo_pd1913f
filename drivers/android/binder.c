...
#ifdef CONFIG_RSC_BINDER_EVENT_NOTI
extern void notify_app_binder_freez(int type, int src_uid, int src_pid, int dst_uid, int dst_pid);
...
#ifdef CONFIG_RSC_BINDER_EVENT_NOTI
		if (!(tr->flags & TF_ONE_WAY) && target_proc != NULL) {
			if (cgroup_freezing_or_frozen(target_proc->tsk) && task_uid(target_proc->tsk).val > SYSTEM_UID &&
				task_uid(proc->tsk).val != task_uid(target_proc->tsk).val) {
				pr_info("bind notify from uid %d pid %d -> uid %d pid %d\n",
					task_uid(proc->tsk).val, proc->tsk->pid, task_uid(target_proc->tsk).val, target_proc->tsk->pid);
				notify_app_binder_freez(0, task_uid(proc->tsk).val, proc->tsk->pid,
					task_uid(target_proc->tsk).val, target_proc->tsk->pid);
			}
		}
#endif
		if (!target_node) {
...
/* vivo chenwenbin add for frozen3.2 begin */
#ifdef CONFIG_RSC_BINDER_EVENT_NOTI
	if (target_proc->tsk && task_uid(target_proc->tsk).val > SYSTEM_UID && cgroup_freezing_or_frozen(target_proc->tsk)) {
		size_t des_free_async_space = binder_alloc_get_free_async_space(&target_proc->alloc);
		size_t count = binder_alloc_get_allocated_count(&target_proc->alloc);
		if (count > 100) {
			binder_user_error("code %d src: %d : %d targer:process %16s uid: %d pid: %d num buffers: %d  space: %d\n", tr->code, proc->tsk->pid, current->pid, target_proc->tsk->comm, task_uid(target_proc->tsk).val, target_proc->pid, count, des_free_async_space);
		}
		if (count > 1000 || des_free_async_space < BINDER_FREE_SPACE_THD) {
			binder_user_error("targer:uid: %d pid: %d num buffers: %d  space: %d\n", task_uid(target_proc->tsk).val, target_proc->pid, count, des_free_async_space);
			notify_app_binder_freez(1, task_uid(proc->tsk).val, proc->tsk->pid,
				task_uid(target_proc->tsk).val, target_proc->tsk->pid);
		}
	}
#endif
/* vivo chenwenbin add for frozen3.2 end */
