...
static bool mtk_dsi_doze_state(struct mtk_dsi *dsi)
{
	...
	if (!crtc->state)
		return 0;
	DDPINFO("%s: crtc: 0x%p state: 0x%p\n", __func__,  crtc, crtc->state);
...
static void mtk_dsi_encoder_disable(struct drm_encoder *encoder)
{
	...
	DDPINFO("%s\n", __func__);
	DRM_ERROR("mtk_dsi_encoder_disable,index = %d,data = MTK_DISP_BLANK_POWERDOWN", index);
	mtk_drm_idlemgr_kick(__func__, crtc, 0);
	if (index == 0)
...
int mtk_dsi_esd_read(struct mtk_ddp_comp *comp, void *handle, uintptr_t slot)
{
	...
	for (i = 0 ; i < ESD_CHECK_NUM ; i++) {
		...
		if (params->lcm_esd_check_table[i].before_esd_check_cmdq) {
			params->lcm_esd_check_table[i].before_esd_check_cmdq(dsi,
				...
				const struct mipi_dsi_msg *msg)
{
	...
	for (i = 0; i < msg->tx_len; i++) {
		...
		DDPMSG("set cmdqaddr %lx, val:%x, mask %x\n", goto_addr,
			tx_buf[i] << ((goto_addr & 0x3u) * 8),
			(0xFFu << ((goto_addr & 0x3u) * 8)));
	}
	if (msg->tx_len > 2)
