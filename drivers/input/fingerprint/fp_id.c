...
static int fp_id_probe(struct platform_device *pdev)
{
	...
	if ((!strncmp(fp_project_name, "PD2080", 6)) || (!strncmp(fp_project_name, "PD2083F_EX", 10)) || (!strncmp(fp_project_name, "PD2083", 6))) {
		fp_id = GOODIX_GF9578;
		pr_info("direct return GOODIX_GF9578 \n");
		return 0;
	}
	...
	if ((!strncmp(fp_project_name, "PD2166", 6))) {
		ret = vreg_setup(&pdev->dev, "vfp", true);
		if (ret) {
			pr_info("%s:can not set vreg", __func__);
			return -EINVAL;
		}
	...
	if ((!strncmp(fp_project_name, "PD2031", 6))) {
		...
		if ((fp_up == 0) && (fp_down == 0)) {
			...
			|| (!strncmp(fp_project_name, "PD2138F_EX", 10)) || (!strncmp(fp_project_name, "PD2147F_EX", 10)) || (!strncmp(fp_project_name, "PD2103F_EX", 10)) || (!strncmp(fp_project_name, "PD2166", 6))) {
		if ((fp_up == 0) && (fp_down == 0)) {
			fp_id = GOODIX_GF3626;
			ret = select_pin_ctl(&pdev->dev, "fp_gpio_pull_down");
			pr_info(" sidefp return GOODIX_GF3626: 0x%x", fp_id);
			if (ret) {
				pr_err("%s: set fp-id pull down error,get gpio value = %d", __func__, gpio_get_value(fp_gpio));
				return -EINVAL;
			}
			...
			fp_id = FPC_FPC1540;
			ret = select_pin_ctl(&pdev->dev, "fp_gpio_pull_down");
			pr_info("sidefp return FPC_FPC1540: 0x%x", fp_id);
			if (ret) {
				pr_err("%s: set fp-id pull down error,get gpio value = %d", __func__, gpio_get_value(fp_gpio));
				return -EINVAL;
			}
		...
		if ((fp_up == 1) && (fp_down == 0)) {
			fp_id = GOODIX_GF9578;
			ret = select_pin_ctl(&pdev->dev, "fp_gpio_pull_down");
			pr_info("PD2031 return GOODIX_GF9578: 0x%x\n", fp_id);
			if (ret) {
			...
			fp_id = SILEAD_GSL7001;
			ret = select_pin_ctl(&pdev->dev, "fp_gpio_pull_up");
			pr_info("PD2031 return SILEAD_GSL7001: 0x%x\n", fp_id);
			if (ret) {
	...
	if ((!strncmp(fp_project_name, "PD2166", 6))) {
		ret = vreg_setup(&pdev->dev, "vfp", false);
		if (ret) {
			pr_info("%s:can not set vreg", __func__);
			return -EINVAL;
		}
