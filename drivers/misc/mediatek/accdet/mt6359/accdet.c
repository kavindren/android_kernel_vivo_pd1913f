...
#ifdef CONFIG_MTK_TYPEC_ACCDET
extern bool get_cc1_cc2_from_tcpc_dev_temporary(u8 *cc1, u8 *cc2);
#endif
...
static inline void check_cable_type(void)
{
	...
	case PLUG_OUT:
		if (cur_AB != ACCDET_STATE_AB_01) {
			pr_notice("accdet %s(), cur_AB need to re-check\n", __func__);
			for (i = 0; i < 4; i++) {
				usleep_range(25*1000, 26*1000);
				cur_AB = pmic_read(PMIC_ACCDET_MEM_IN_ADDR) >> ACCDET_STATE_MEM_IN_OFFSET;
				cur_AB = cur_AB & ACCDET_STATE_AB_MASK;
				pr_notice("accdet %s(), cur_AB is %d (%d times)\n", __func__, cur_AB, i);
				if (cur_AB == ACCDET_STATE_AB_01) {
					break;
				}
			}
...
#endif
{
	...
	if (eint_accdet_sync_flag) {
		if (pre_cable_type != cable_type) {
			...
			if (max20328_is_ready) {
				max20328_switch_mode_event(MAX_USBC_AUIDO_HP_ON);
				/*  wangkai add for ESD to restore 0x0D and 0x0E's regs value*/
				max20328_save_0x0D_0x0E_reg();
			}
		}
	} else
...
#ifdef CONFIG_MTK_TYPEC_ACCDET
		cc_flag = get_cc1_cc2_from_tcpc_dev_temporary(&cc1, &cc2);
		pr_info("%s cc_flag is %d\n", __func__, cc_flag);
		if (cc_flag && (cc1 == 0) && (cc2 == 0)) {
			pr_info("%s no need accdet work, cc1 = %d, cc2 = %d\n", __func__, cc1, cc2);
		} else {
			pr_info("%s need accdet work\n", __func__);
			accdet_queue_work();
		}
#else
		accdet_queue_work();
