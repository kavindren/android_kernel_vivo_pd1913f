...
#define get_cluster_cpufreq(id, idx)	\
		((ppm_main_info.cluster_info[id].dvfs_tbl)	\
		? ppm_main_info.cluster_info[id].dvfs_tbl[idx].frequency	\
		: ~0)

	/* add notify cpucap here, trans freqidx to freq, construct a policy struct arg */
	{
		struct rsc_cpucap_adj_t adj;
		static int last_max_freq_idx[NR_PPM_CLUSTERS] = {DVFS_OPP_NUM};

		if (max_freq_idx != last_max_freq_idx[cluster]) {
			last_max_freq_idx[cluster] = max_freq_idx;
			max_freq_idx = (max_freq_idx == -1) ? 0 : max_freq_idx;
			memset(&adj, 0, sizeof(adj));
			adj.mode = LMH_SETLIM_ADJUST_FREQ;
			adj.max_mitigated_freq = get_cluster_cpufreq(cluster, max_freq_idx);
			adj.cpu = smp_processor_id();
			arch_get_cluster_cpus(&adj.cpumask, cluster);
			/*send adjust message! we have make sure that
			   there is no any lock in fucntion rsc_cpucap_adjust*/
			rsc_cpucap_adjust(&adj);
		}
	}
#endif
