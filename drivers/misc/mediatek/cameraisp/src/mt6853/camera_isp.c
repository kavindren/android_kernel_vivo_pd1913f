...
extern long ois_interface_dispatcher_sub(unsigned int ioc_command, __user void *buf);
...
#ifdef CONFIG_MTK_CAM_PD2083F_EX
	if (IrqStatus & SOF_INT_ST) {
		//LOG_INF("irq type %lu status 0x%08x vsync %d sofcount %llu cardinalNum %llu %llu",
		//	irq_module, IrqStatus, cur_v_cnt, sof_count[module], cardinalNum, Vsync_cnt[cardinalNum]);
		...
		ois_interface_dispatcher_sub(AFICO_X_OIS_VSYNC, (void *)(&gVsyncInfo));
