 ...
 *****************************************************************************/
#if defined(CONFIG_MTK_CAM_PD2135)
extern long ois_interface_dispatcher_tele(unsigned int ioc_command, __user void *buf);
#endif
...
/*chenhan  add for ois*/
static struct ois_vsync_info gVsyncInfo = { 0, };
/*add end*/
#endif
...
irqreturn_t ISP_Irq_CAM(enum ISP_IRQ_TYPE_ENUM irq_module)
{
	...
	if (IrqStatus & SOF_INT_ST) {
		/*
		LOG_INF("module %d irq module %d reg module %lu status 0x%08x", module, irq_module, reg_module, IrqStatus);
		LOG_INF("vsync %d sofcount %lu cardinalNum %lu %ld", cur_v_cnt, sof_count[module], cardinalNum, Vsync_cnt[cardinalNum]);
		*/
		gVsyncInfo.vsync_cnt = (u64)cur_v_cnt;
		gVsyncInfo.sof_timestamp_boot = ktime_get_boot_ns();
		gVsyncInfo.module_idx = (u64)module;
		ois_interface_dispatcher(AFICO_X_OIS_VSYNC, (void *)(&gVsyncInfo));
		ois_interface_dispatcher_tele(AFICO_X_OIS_VSYNC, (void *)(&gVsyncInfo));
	}
	/*add end*/
#endif
