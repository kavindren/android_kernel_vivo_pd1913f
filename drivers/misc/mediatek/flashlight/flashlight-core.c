...
static int fl_set_level(struct flashlight_dev *fdev, int level)
{
	...
	if (fdev->ops->flashlight_ioctl(fdev->client,FLASH_IOC_SET_DUTY,
...
static int fl_set_led_state(struct flashlight_dev *fdev, int state)
{
	struct flashlight_dev_arg fl_dev_arg;
	if (!fdev || !fdev->ops) {
		pr_info("Failed with no flashlight ops\n");
	...
	/* ioctl */
	fl_dev_arg.channel = fdev->dev_id.channel;
	fl_dev_arg.type = fdev->dev_id.type;
	fl_dev_arg.ct = fdev->dev_id.ct;
	fl_dev_arg.part = fdev->dev_id.part;
	fl_dev_arg.arg = state;

 

	if (fdev->ops->flashlight_ioctl(fdev->client, FLASH_IOCTL_SET_LED_STATE,
				(unsigned long)&fl_dev_arg)) {
		pr_err("Failed to set level\n");
		return -EFAULT;
	}
...
static int fl_enable(struct flashlight_dev *fdev, int enable)
{
	...
	if (fdev->ops->flashlight_ioctl(fdev->client, FLASH_IOC_SET_ONOFF,
				(unsigned long)&fl_dev_arg)) {
		...
		const int type, const int ct, const int part)
{
	...
	list_for_each_entry(fdev, &flashlight_list, node) {
		if (fdev->dev_id.type == 1)
			count = count + 1;
	}
	pr_info("count = %d %d %d %d",count, type, ct,part);
	/* return the first flashlight device */
	list_for_each_entry(fdev, &flashlight_list, node) {
		if(count == FRONT_FLASHLIGHT_FOUR_LED && fdev->dev_id.type == 1){
...
int flashlight_dev_register_by_device_id_and_i2c_client(
		struct flashlight_device_id *dev_id, struct i2c_client * client, 
		struct flashlight_operations *dev_ops)
{
	struct flashlight_dev *fdev;
...
EXPORT_SYMBOL(flashlight_dev_register_by_device_id_and_i2c_client);
...
static int flashlight_update_charger_status(struct flashlight_dev *fdev)
{
	...
	/* ioctl */
	fl_dev_arg.channel = fdev->dev_id.channel;
	if (fdev->ops->flashlight_ioctl(fdev->client, FLASH_IOC_IS_CHARGER_READY,
				(unsigned long)&fl_dev_arg))
...
static int pt_trigger(void)
{
	...
	list_for_each_entry(fdev, &flashlight_list, node) {
		...
		fdev->ops->flashlight_set_driver(fdev->client, 1);
		if (pt_strict) {
		...
		fdev->ops->flashlight_set_driver(fdev->client,0);
		...
		struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	fdev = flashlight_find_dev_by_full_index(
			...
			flashlight_get_part_index(fl_arg.part_id));
	...
	case FLASH_IOCTL_SET_LED_STATE:
		pr_info("FLASH_IOCTL_SET_LED_STATE(%d,%d,%d)\n",
				type, ct, part);

		mutex_lock(&fl_mutex);
		ret = fl_set_led_state(fdev, fl_arg.arg);
		mutex_unlock(&fl_mutex);				
		break;
	...
	case FLASHLIGHTIOC_X_SET_DRIVER:
		pr_debug("FLASHLIGHTIOC_X_SET_DRIVER(%d,%d,%d): %d\n",
				type, ct, part, fl_arg.arg);
		if (fdev->ops) {
			ret = fdev->ops->flashlight_set_driver(fdev->client,fl_arg.arg);
			if (fdev->dev_id.decouple) {
				fl_dev_arg.arg = FLASHLIGHT_SCENARIO_DECOUPLE;
				fdev->ops->flashlight_ioctl(fdev->client,
	...
	case FLASH_IOC_SET_SCENARIO:
		...
		if (fdev->ops) {
			if (fdev->dev_id.decouple)
				fl_dev_arg.arg |= FLASHLIGHT_SCENARIO_DECOUPLE;
			ret = fdev->ops->flashlight_ioctl(fdev->client,
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (fdev->ops) {
		fdev->ops->flashlight_strobe_store(fdev->client ,fl_arg);
		ret = size;
	} else {
		pr_info("Failed with no flashlight ops\n");
		ret = -1;
		...
		struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	list_for_each_entry(fdev, &flashlight_list, node) {
		...
		fl_dev_arg.arg = -1;
		fdev->ops->flashlight_ioctl(fdev->client,FLASH_IOC_GET_HW_TIMEOUT,
				(unsigned long)&fl_dev_arg);
		hw_timeout = fl_dev_arg.arg;

		fl_dev_arg.arg = -1;
		fdev->ops->flashlight_ioctl(fdev->client,FLASH_IOC_GET_DUTY_NUMBER,
				(unsigned long)&fl_dev_arg);
		max_duty = fl_dev_arg.arg - 1;

		fl_dev_arg.arg = -1;
		fdev->ops->flashlight_ioctl(fdev->client,FLASH_IOC_GET_MAX_TORCH_DUTY,
				(unsigned long)&fl_dev_arg);
		...
		struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (fdev && fdev->ops) {
		...
		for (i = 0; i < duty_num; i++) {
			fl_dev_arg.arg = i;
			if (fdev->ops->flashlight_ioctl(fdev->client,
		...
		struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	list_for_each_entry(fdev, &flashlight_list, node) {
		...
		fl_dev_arg.arg = -1;
		fdev->ops->flashlight_ioctl(fdev->client,FLASH_IOC_GET_HW_FAULT,
				(unsigned long)&fl_dev_arg);
		fault_flag1 = fl_dev_arg.arg;

		fl_dev_arg.arg = -1;
		fdev->ops->flashlight_ioctl(fdev->client,FLASH_IOC_GET_HW_FAULT2,
				(unsigned long)&fl_dev_arg);
