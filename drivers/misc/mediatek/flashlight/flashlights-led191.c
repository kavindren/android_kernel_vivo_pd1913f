 ...
 *****************************************************************************/
static int led191_ioctl(struct i2c_client * client, unsigned int cmd, unsigned long arg)
{
...
static int led191_set_driver(struct i2c_client * client, int set)
{
	int ret = 0;
...
static ssize_t led191_strobe_store(struct i2c_client * client, struct flashlight_arg arg)
{
	led191_set_driver(client, 1);
	led191_set_level(arg.level);
	led191_timeout_ms = 0;
	led191_enable();
	msleep(arg.dur);
	led191_disable();
	led191_set_driver(client, 0);
