 ...
 *****************************************************************************/
static int lm3642_ioctl(struct i2c_client * client, unsigned int cmd, unsigned long arg)
{
...
static int lm3642_set_driver(struct i2c_client * client, int set)
{
	int ret = 0;
...
static ssize_t lm3642_strobe_store(struct i2c_client * client, struct flashlight_arg arg)
{
	lm3642_set_driver(client, 1);
	lm3642_set_level(arg.level);
	lm3642_timeout_ms = 0;
	lm3642_enable();
	msleep(arg.dur);
	lm3642_disable();
	lm3642_set_driver(client, 0);
