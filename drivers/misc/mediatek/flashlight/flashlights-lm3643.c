 ...
 *****************************************************************************/
static int lm3643_ioctl(struct i2c_client * client, unsigned int cmd, unsigned long arg)
{
...
static int lm3643_set_driver(struct i2c_client * client, int set)
{
	int ret = 0;
...
static ssize_t lm3643_strobe_store(struct i2c_client * client, struct flashlight_arg arg)
{
	lm3643_set_driver(client, 1);
	lm3643_set_level(arg.channel, arg.level);
	lm3643_timeout_ms[arg.channel] = 0;
	lm3643_enable(arg.channel);
	msleep(arg.dur);
	lm3643_disable(arg.channel);
	lm3643_set_driver(client, 0);
