...
static const unsigned char ktd2687_torch_level[LM3644_LEVEL_NUM] = {
	0x15, 0x22, 0x2B, 0x32, 0x42, 0x51, 0x5E, 0x00, 0x00, 0x00,
...
static const unsigned char ktd2687_torch_level[LM3644_LEVEL_NUM] = {
	0x15, 0x22, 0x2B, 0x32, 0x3F, 0x42, 0x5E, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static const unsigned char ktd2687_torch_level[LM3644_LEVEL_NUM] = {
	0x15, 0x19, 0x21, 0x29, 0x2E, 0x43, 0x52, 0x7F, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
 ...
 */
static int set_flashlight_state(struct i2c_client * client, int ct, int part, int state)
{
...
#if defined  (CONFIG_MTK_CAM_PD2083F_EX)
			if (chip->device_id == AW36413_DEVIC_ID_VALUE)
				lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x19);/*(Brightnees code x 1.4mA)+0.997ma Torch 0x1A = 78mA*/
			else
...
#if defined  (CONFIG_MTK_CAM_PD2083F_EX)
			if (chip->device_id == AW36413_DEVIC_ID_VALUE){
				lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x19);/*write reg5 bit 7 to 0*/
				lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED2, 0x19);/*default 0x33 = 150mA 0x1A = 78mA (Brightness Code*2.91mA)+2.55mA Torch*/
			}else{
		...
		if(fl_ic_number == FRONT_FLASHLIGHT_DOUBLE_LED) {
			...
			chip->lm3644_reg_enable |= LM3644_ENABLE_LED1_TORCH;
				...
				if (chip->device_id == AW36413_DEVIC_ID_VALUE) {
					...
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x19);/*(Brightness Code*2.91mA)+2.55mA Torch*/
				} else {
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x35);/*(Brightnees code x 1.4mA)+0.997ma Torch*/
				}
			#else
				if (chip->device_id == AW36413_DEVIC_ID_VALUE) {
					pr_info("BBK_TORCH_LOW, AW36413_REG_TORCH_LEVEL_LED1 = 0x19\n");
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x19);/*aw36413 0x19 = 75mA (Brightness Code x 2.91mA)+2.55mA Torch*/
				} else {
					pr_info("BBK_TORCH_LOW, LM3644_REG_TORCH_LEVEL_LED1 = 0x35\n");
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x35);/*KTD2687 0x35 = 75mA (Brightnees code x 1.4mA)+0.997ma Torch*/
				}
			#endif
		}
		lm3644_write_reg(client, LM3644_REG_ENABLE, chip->lm3644_reg_enable);
	...
	case FRONT_TORCH_ON:
		...
		if(fl_ic_number == FRONT_FLASHLIGHT_DOUBLE_LED) {
			...
			chip->lm3644_reg_enable |= LM3644_ENABLE_LED2_TORCH;
				...
				if (chip->device_id == AW36413_DEVIC_ID_VALUE){
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x19);/*write reg5 bit 7 to 0*/
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED2, 0x19);/*default 0x33 = 150mA (Brightness Code*2.91mA)+2.55mA Torch*/
				}else{
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x35);/*write reg5 bit 7 to 0*/
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED2, 0x6A);/*default 2E == 65mA  0x6A = 150mA(Brightnees code x 1.4mA)+0.997ma Torch*/
				}
			#else
				if (chip->device_id == AW36413_DEVIC_ID_VALUE){
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x22);/*write reg5 bit 7 to 0*/
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED2, 0x22);/*default 0x33 = 150mA (Brightness Code*2.91mA)+2.55mA Torch*/
				}else{
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED1, 0x35);/*write reg5 bit 7 to 0*/
					lm3644_write_reg(client, LM3644_REG_TORCH_LEVEL_LED2, 0x6A);/*default 2E == 65mA  0x6A = 150mA(Brightnees code x 1.4mA)+0.997ma Torch*/
				}
...
static int lm3644_ioctl(struct i2c_client *client, unsigned int cmd, unsigned long arg)
{
    ...
    case FLASH_IOCTL_SET_LED_STATE:
		...
		set_flashlight_state(client, fl_arg->ct, fl_arg->part, led_state);
		break;
...
static ssize_t lm3644_strobe_store(struct i2c_client * client,struct flashlight_arg arg)
{
	...
	if (arg.channel == LM3644_CHANNEL_CH2 && arg.level == 27){
		...
		if(arg.dur == 300)
			set_flashlight_state(client, arg.ct, arg.part, 11);
	}else{
