 ...
 *****************************************************************************/
static int rt4505_ioctl(struct i2c_client * client, unsigned int cmd, unsigned long arg)
{
...
static int rt4505_set_driver(struct i2c_client * client, int set)
{
	int ret = 0;
...
static ssize_t rt4505_strobe_store(struct i2c_client * client, struct flashlight_arg arg)
{
	rt4505_set_driver(client, 1);
	rt4505_set_level(arg.level);
	rt4505_timeout_ms = 0;
	rt4505_enable();
	msleep(arg.dur);
	rt4505_disable();
	rt4505_set_driver(client, 0);
