...
enum ACDK_SENSOR_FEATURE_ENUM {
	...
	SENSOR_FEATURE_GET_RAW_INFO, //add by zhuxianwei
	...
	SENSOR_FEATURE_GET_CUSTOM_INFO,   /*vivo hope add 2018.3.3*/
	...
	SENSOR_FEATURE_SET_SAT_MODE,
	SENSOR_FEATURE_SET_TG_IDX,
...
struct SET_PD_BLOCK_INFO_T {
	...
	// vivo xuyuanwen add for algo need get pdaf coordinator start
	MUINT32 i4PosLVivo[16][2];	/* left pd pixel position in one block*/
	MUINT32 i4PosRVivo[16][2];	/* right pd pixel position in one block*/
	// vivo xuyuanwen add for algo need get pdaf coordinator end

	/* vivo xuyuanwen add for start offset of first PD block start*/
	MUINT32 i4OffsetXVivo;
	MUINT32 i4OffsetYVivo;
	/* vivo xuyuanwen add for start offset of first PD block end*/
};
...
struct SENSOR_FUNCTION_STRUCT {
	...
	MUINT32 (*SensorClose)(void);
	MUINT32 (*SensorHop)(int simid, MUINT8 rat, MUINT32 arfcn);//vivo changcheng porting
  	MUINT32 (*SensorDoHop)(int flag, void *psensor);//vivo changcheng porting
