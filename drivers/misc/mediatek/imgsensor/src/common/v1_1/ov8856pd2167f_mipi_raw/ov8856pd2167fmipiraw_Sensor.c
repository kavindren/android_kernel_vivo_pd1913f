...
//#include "ov8856pd2167fmipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV8856PD2167F_SENSOR_ID,
	...
	.custom6 = { /*user for dualcam*/
		...
		/*	 following for GetDefaultFramerateByScenario()	*/
		.max_framerate = 300,
		.mipi_pixel_rate = 162930000,//273870000,
		},
	.custom7 = {
		.pclk = 143870000,
		.linelength  = 1932,
	...
	.custom10 = {
		...
		.gain_step = 4,
		.gain_type = 1,
		.max_frame_length = 0xfffc,//0x90f7,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[15] = {  
	...
	{ 3264, 2448, 	432,	324, 2400, 1800, 2400, 1800, 0, 0, 2400, 1800,  0,  0, 2400, 1800}, // custom6
	{ 3264, 2448, 	480,    360, 2304, 1728, 2304, 1728, 0, 0, 2304, 1728,  0,  0, 2304, 1728}, // custom7
	{ 3264, 2448, 	0,		0,	 3264, 2448, 3264, 2448, 0, 0, 3264, 2448,  0,  0, 3264, 2448}, // custom8
	{ 3264, 2448, 	0,		0,	 3264, 2448, 3264, 2448, 0, 0, 3264, 2448,  0,  0, 3264, 2448}, // custom9
	{ 3264, 2448, 	0,		0,	 3264, 2448, 3264, 2448, 0, 0, 3264, 2448,  0,  0, 3264, 2448}, // custom10
};
...
extern int vivo_main2_otp_read_ov8856pd2167f(void);
extern otp_error_code_t MAIN2_OTP_ERROR_CODE_OV8856PD2167F;
#endif
MUINT32  sn_inf_main2_ov8856pd2167f[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main2_ov8856pd2167f[4];  
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor_16_8(0x300B) << 8) | read_cmos_sensor_16_8(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				vivo_otp_read_when_power_on = vivo_main2_otp_read_ov8856pd2167f();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN2_885A_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, MAIN2_OTP_ERROR_CODE_OV8856PD2167F);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	custom8_setting();/*using preview setting*/
	set_mirror_flip(imgsensor.mirror);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	custom9_setting();/*using preview setting*/
	set_mirror_flip(imgsensor.mirror);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	custom10_setting();/*using preview setting*/
	set_mirror_flip(imgsensor.mirror);
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = MAIN2_OTP_ERROR_CODE_OV8856PD2167F;//otp_state
				memcpy( feature_data+2, sn_inf_main2_ov8856pd2167f, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main2_ov8856pd2167f, sizeof(MUINT32)*4);
				#if 0
					for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main2_ov8856pd2167f[%d]= 0x%x\n", i, sn_inf_main2_ov8856pd2167f[i]);
					}
				#endif
			}
				break;
...
UINT32 OV8856PD2167F_MIPI_RAW_SensorInit(
