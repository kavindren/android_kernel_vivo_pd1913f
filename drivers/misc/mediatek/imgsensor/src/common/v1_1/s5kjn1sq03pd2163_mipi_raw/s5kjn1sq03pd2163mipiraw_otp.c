...
#include "s5kjn1sq03pd2163mipiraw_Sensor.h"
...
#define VIVO_OTP_DATA_SIZE 0x3B50 //0x3AE9  /*sizeof(moduleinfo)+sizeof(awb)+sizeof(af)+sizeof(lsc)*/
...
#define VIVO_VENDOR_DRIVERIC_ID 0x05 //0x07
...
static unsigned const int PDAF_proc2_checksum_addr = 0x1A3F; //0x1A3E;
static unsigned const int XTC_cal_addr = 0x1A40; //0x1A3F;
static unsigned const int XTC_cal_checksum_addr = 0x248F; //0x2474;
static unsigned const int SensorXTC_cal_addr = 0x2490; //0x2475;
static unsigned const int SensorXTC_cal_checksum_addr = 0x279F; //0x2776;
static unsigned const int PDXTC_addr = 0x27A0; //0x2777;
static unsigned const int PDXTC_checksum_addr = 0x374F; //0x3718;
static unsigned const int SW_GGC_addr = 0x3750; //0x3719;
static unsigned const int SW_GGC_checksum_addr = 0x39DF; //0x398C;
static unsigned const int HWGGC_addr = 0x39E0; //0x398D;
static unsigned const int HWGGC_addr_checksum_addr = 0x3B4F; //0x3AE8;
#if 0
...
extern MUINT32  af_calib_inf_main_s5kjn1sq03pd2163[6];
extern MUINT32 crosstalk_data_PD2163[8006];  /*0 flag   1-606 data*/
...
int MAIN_38E1_otp_read(void)
{
	...
	if (check_if_group_valid == 0x01) { /****all the data is valid****/
		...
		af_calib_inf_main_s5kjn1sq03pd2163[0] = (MUINT32)vivo_otp_data_s5kjn1sq03[Af_addr + 7];
		af_calib_inf_main_s5kjn1sq03pd2163[1] = (MUINT32)vivo_otp_data_s5kjn1sq03[Af_addr + 8];
		af_calib_inf_main_s5kjn1sq03pd2163[2] = (MUINT32)vivo_otp_data_s5kjn1sq03[Af_addr + 9];
		af_calib_inf_main_s5kjn1sq03pd2163[3] = (MUINT32)vivo_otp_data_s5kjn1sq03[Af_addr + 10];
		af_calib_inf_main_s5kjn1sq03pd2163[4] = (MUINT32)vivo_otp_data_s5kjn1sq03[Af_addr + 11];
		af_calib_inf_main_s5kjn1sq03pd2163[5] = (MUINT32)vivo_otp_data_s5kjn1sq03[Af_addr + 12];
		LOG_INF("af1 = %d, af2= %d af3= %d af4= %d af5= %d af6= %d\n",af_calib_inf_main_s5kjn1sq03pd2163[0],af_calib_inf_main_s5kjn1sq03pd2163[1],af_calib_inf_main_s5kjn1sq03pd2163[2],af_calib_inf_main_s5kjn1sq03pd2163[3],af_calib_inf_main_s5kjn1sq03pd2163[4],af_calib_inf_main_s5kjn1sq03pd2163[5]);
		...
		for (i = 0; i < 2612; i++) {
			crosstalk_data_PD2163[i]=vivo_otp_data_s5kjn1sq03[0x1a41+i];
		}
		...
		for (i = 0; i < 768; i++) {
			crosstalk_data_PD2163[2612+i]=vivo_otp_data_s5kjn1sq03[0x2491+i];
		}
		...
		for (i = 0; i < 4000; i++) {
			crosstalk_data_PD2163[3380+i]=vivo_otp_data_s5kjn1sq03[0x27A1+i];
		}
		...
		for (i = 0; i < 626; i++) {
			crosstalk_data_PD2163[7380+i]=vivo_otp_data_s5kjn1sq03[0x3751+i];
		}
