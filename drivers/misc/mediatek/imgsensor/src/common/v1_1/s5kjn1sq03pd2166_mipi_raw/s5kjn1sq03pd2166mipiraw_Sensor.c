 ...
 * Filename:
 * ---------
 *	 s5kjn1sq03pd2166mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#include "s5kjn1sq03pd2166mipiraw_freq.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJN1SQ03PD2166_SENSOR_ID,
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =  //4080X3072
{
	...
	.i4BlockNumY = 382,
	.iMirrorFlip = 3,
	.i4Crop = { {0,0}, {0,0}, {0, 382}, {0, 0}, {0, 0}, {0,0}, {120, 450}, {0, 0}, {0, 0}, {120, 450}},
};
#endif
#if 1
...
extern otp_error_code_t S5KJN1SQ03PD2166_OTP_ERROR_CODE;
#endif
MUINT32  af_calib_inf_main_s5kjn1sq03pd2166[6];
MUINT32  sn_inf_main_s5kjn1sq03pd2166[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kjn1sq03pd2166[4];
MUINT32  crosstalk_data_PD2166[PDAF_DATA_SIZE - 2];  /*0 flag   1-836 data*/
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjn1sq03pd2166 temperature
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJN1SQ03PD2166_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjn1sq03pd2166, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjn1sq03pd2166, sizeof(MUINT32)*4);
				memcpy( feature_data+12, af_calib_inf_main_s5kjn1sq03pd2166, sizeof(MUINT32)*6);
                #if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_s5kjn1sq03pd2166[%d]= 0x%x\n", i, sn_inf_main_s5kjn1sq03pd2166[i]);
						}
                #endif
			}
				break;
    ...
    case SENSOR_FEATURE_GET_4CELL_DATA:/*get 4 cell data from eeprom*/
        {
            ...
            if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
                ...
                for(i = 0; i < PDAF_DATA_SIZE - 2 ; i++) {
                    data[2 + i] = crosstalk_data_PD2166[i];
                }
                ...
                for(m = PDAF_DATA_SIZE - 2;m >= PDAF_DATA_SIZE - 2 -5 ; m--) {
                    pr_debug("read Cross Talk calibration data size= i= %d data=0x%x\n",m, data[m]);
                }
...
UINT32 S5KJN1SQ03PD2166_MIPI_RAW_SensorInit(
