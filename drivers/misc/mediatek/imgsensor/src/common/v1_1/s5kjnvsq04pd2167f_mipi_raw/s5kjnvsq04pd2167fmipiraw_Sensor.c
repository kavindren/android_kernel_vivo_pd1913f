 ...
 * Filename:
 * ---------
 *	 s5kjnvsq04pd2167fmipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "MAIN[EF38]_camera_sensor"
...
#include "s5kjnvsq04pd2167fmipiraw_freq.h"
...
static int  isOtpSetting    = 0; //default:0 otp setting:1 no-otp:2

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5KJNVSQ04PD2167F_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[15] = {
	...
	{8160, 6144,  240,  912, 7680, 4320, 3840, 2160, 0, 0, 3840, 2160, 0, 0, 3840, 2160}, /* custom6 */
	{8160, 6144,    0,  12, 8160, 6120, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /* custom7 */
	{8160, 6144,    0,  12, 8160, 6120, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /* custom8 */
	{8160, 6144,    0,  12, 8160, 6120, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /* custom9 */
	{8160, 6144,    0,  12, 8160, 6120, 4080, 3060, 0, 0, 4080, 3060, 0, 0, 4080, 3060}, /* custom10 */
};
#if 1
...
static struct SET_PD_BLOCK_INFO_T imgsensor_pd_info =  //4080X3072
{
	...
	.i4BlockNumY = 382,
	.iMirrorFlip = 3,
	.i4Crop = { {0,0}, {0,0}, {0, 382}, {0, 0}, {0, 0}, {0,0}, {120, 450}, {120, 450}, {0, 0}, {0, 0}, {120, 450}, {0, 0}, {0, 0}, {0, 0}, {0, 0}},
};
#endif
#if 1
/*hope add otp check start*/
static int vivo_otp_read_when_power_on;
extern int MAIN_EF38_otp_read(void);
extern otp_error_code_t S5KJNVSQ04PD2167F_OTP_ERROR_CODE;
#endif
MUINT32  sn_inf_main_s5kjnvsq04pd2167f[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_s5kjnvsq04pd2167f[4];
MUINT32  af_calib_inf_main_s5kjnvsq04pd2167f[6];
MUINT32  crosstalk_data_s5kjnvsq04pd2167f[8006];  /*0 flag   1-836 data*/
...
static int s5kjnv_read_framelength;
static int s5kjnv_read_shutter;
static int s5kjnv_read_gain;
#endif
...
#ifdef READ_FRAMELENGTH_SHUTTER_GAIN
	s5kjnv_read_framelength = read_cmos_sensor_16_16(0x0340);
	s5kjnv_read_shutter = read_cmos_sensor_16_16(0x0202);
	PK_DBG(" s5kjnv_read_shutter = %d, s5kjnv_read_framelength = %d \n", s5kjnv_read_shutter,s5kjnv_read_framelength);
#endif
}	/*	write_shutter  */
...
#ifdef READ_FRAMELENGTH_SHUTTER_GAIN
	s5kjnv_read_gain = read_cmos_sensor_16_16(0x0204);
	PK_DBG(" s5kjnv_read_gain = %d \n", s5kjnv_read_gain);
#endif
...
static kal_uint16 addr_data_pair_init_non_otp[] = {
	0x6028, 0x2400,
	0x602A, 0x7700,
	0x6F12, 0x1753,
	0x6F12, 0x01FC,
...
static kal_uint16 addr_data_pair_init[] = {
	...
	0x602A, 0x1A0E,
	0x6F12, 0x9600,
	//0x602A, 0x21A6, // close for no sof 20211014
	//0x6F12, 0x3240, // close for no sof 20211014
	0x6028, 0x4000,
	0xF44E, 0x0011,
	0xF44C, 0x0B0B,
	0xF44A, 0x0006,
...
static void sensor_init(void)
{
	...
	write_cmos_sensor_16_16(0x0000,0xEF38);
	write_cmos_sensor_16_16(0x001E,0x000A);
	write_cmos_sensor_16_16(0x6028,0x4000);
	...
	if(1 == isOtpSetting) {
		PK_DBG("sensor_init otp setting \n");
	table_write_cmos_sensor(addr_data_pair_init,
		   sizeof(addr_data_pair_init) / sizeof(kal_uint16));
	} else if(2 == isOtpSetting) {
		PK_DBG("sensor_init non-otp setting \n");
		table_write_cmos_sensor(addr_data_pair_init_non_otp,
		   sizeof(addr_data_pair_init_non_otp) / sizeof(kal_uint16));
	}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = read_cmos_sensor_16_16(0x0000);
			printk("read out JNVSQ04 sensor id 0x%x \n",*sensor_id);

			// zhangyinpeng add for diff jnv otp setting and no-otp setting start
			sensor_version = read_cmos_sensor_16_16(0x000E);
			//0x040F, 0x0411, 0x0413, 0x0414 no-otp setting
			//0x0433, 0x0434, 0x0443, 0x0444 otp setting
			if((sensor_version == 0x0433) || (sensor_version == 0x0434) || (sensor_version == 0x0443) || (sensor_version == 0x0444)) {
				isOtpSetting = 1;
			}
			else if ((sensor_version == 0x040F) || (sensor_version == 0x0411) || (sensor_version == 0x0413) || (sensor_version == 0x0414)) {
				isOtpSetting = 2;
			}
			// zhangyinpeng add for diff jnv otp setting and no-otp setting end

			if (( *sensor_id == imgsensor_info.sensor_id) || ( *sensor_id == 0x38ef)) { //compatible jnv\jn1
				...
				vivo_otp_read_when_power_on = MAIN_EF38_otp_read();
				pr_debug("read eeprom ---vivo_otp_read_when_power_on = %d,MAIN_F8D1_OTP_ERROR_CODE=%d\n", vivo_otp_read_when_power_on, S5KJNVSQ04PD2167F_OTP_ERROR_CODE);
				/*vivo lxd add end*/
				return ERROR_NONE;
			}
			printk("Read sensor id fail, id: 0x%x, %x\n", imgsensor.i2c_write_id, imgsensor_info.sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
	    		sensor_id = read_cmos_sensor_16_16(0x0000);
			PK_DBG("read out JNVSQ04 sensor id 0x%x \n", sensor_id);
			if (( sensor_id == imgsensor_info.sensor_id) || ( sensor_id == 0x38ef)) { //compatible jnv\jn1
	...
	if ((imgsensor_info.sensor_id != sensor_id) && (sensor_id != 0x38ef)) //compatible jnv\jn1
		return ERROR_SENSOR_CONNECT_FAIL;
...
#ifdef HOPE_ADD_FOR_CAM_TEMPERATURE_NODE  //add for s5kjnvsq04pd2167f temperature
static kal_uint32 get_sensor_temperature(void)
{
	...
	if (TMC_Shift_Value > 0x7FFF ){
		...
		if (TMC_Value > 100){
			PK_DBG("s5kjnvsq04 TMC 0X000A temperature Value=%d, cause of TMC > 100 \n", TMC_Value);
			TMC_Value = 0;
		}
		sensor_temperature[0] = TMC_Value;

		if (__ratelimit(&ratelimit))
			PK_DBG("s5kjnvsq04 TMC 0X000A temperature Value=0x%x, sensor_temperature[0] =%d\n", TMC_Value, sensor_temperature[0]);
	}
    ...
    */
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
				*(MUINT32 *)(uintptr_t)(*(feature_data+1)) = S5KJNVSQ04PD2167F_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_s5kjnvsq04pd2167f, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_main_s5kjnvsq04pd2167f, sizeof(MUINT32)*4);
				memcpy( feature_data+12, af_calib_inf_main_s5kjnvsq04pd2167f, sizeof(MUINT32)*6);
			#if 0
				for (i = 0 ; i<12 ; i++ ){
					printk("sn_inf_main_s5kjnvsq04pd2167f[%d]= 0x%x\n", i, sn_inf_main_s5kjnvsq04pd2167f[i]);
				}
			#endif
			}
				break;
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:/* get 4 cell data from eeprom*/
	{
		...
		if (type == FOUR_CELL_CAL_TYPE_XTALK_CAL) {
			...
			data[1] = 0x1F;
			for(i = 0; i< 8006 ; i++)
				data[2 + i] = crosstalk_data_s5kjnvsq04pd2167f[i];
...
UINT32 S5KJNVSQ04PD2167F_MIPI_RAW_SensorInit(
