...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846PD2163_SENSOR_ID,
	...
	.hs_video = {/*no use*/
		.pclk = 287700000,				//record different mode's pclk
		.linelength  = 3800,				//record different mode's linelength
		.framelength = 1261,			/*record different mode's framelength*/
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
		.grabwindow_width  = 1920,   /*2592 record different mode's width of grabwindow*/
		.grabwindow_height = 1080,	 /*1944record different mode's height of grabwindow*/
		/*	 following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario	*/
		.mipi_data_lp2hs_settle_dc = 14,
		.mipi_pixel_rate = 287732000,
		.max_framerate = 600,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[10] =
{
	...
	{ 3264, 2448, 	336,	252,  2592, 1944, 2592, 1944, 0, 0, 2592, 1944,  0,  0, 2592, 1944}, // custom1
...
extern int sub2_846_otp_read(void);
int sub2_otp_read_when_power_on;
extern otp_error_code_t SUB2_846_OTP_ERROR_CODE;
MUINT32  sn_inf_sub2_hi846[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_sub2_hi846[4];
			...
			do {
				*sensor_id = return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					...
					/*vivo caofeixiang add for CameraEM otp errorcode*/
					PK_DBG("cfx_add:start read eeprom ---sub2_otp_read_when_power_on = %d\n", sub2_otp_read_when_power_on);
					sub2_otp_read_when_power_on = sub2_846_otp_read();
					PK_DBG("cfx_add:end read eeprom ---sub2_otp_read_when_power_on = %d,SUB2_846_OTP_ERROR_CODE =%d\n", sub2_otp_read_when_power_on,SUB2_846_OTP_ERROR_CODE);
					/*vivo caofeixiang add end*/
					return ERROR_NONE;
				}
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		PK_DBG("SENSOR_FEATURE_GET_CUSTOM_INFO information type:%lld  SUB2_846_OTP_ERROR_CODE:%d \n", *feature_data,SUB2_846_OTP_ERROR_CODE);
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = SUB2_846_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_sub2_hi846, sizeof(MUINT32)*13);
				memcpy( feature_data+10, material_inf_sub2_hi846, sizeof(MUINT32)*4);
				#if 0
					for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_sub2_hi846[%d]= 0x%x\n", i, sn_inf_sub2_hi846[i]);
					}
				#endif
				}
			break;
...
UINT32 HI846PD2163_MIPI_RAW_SensorInit(
