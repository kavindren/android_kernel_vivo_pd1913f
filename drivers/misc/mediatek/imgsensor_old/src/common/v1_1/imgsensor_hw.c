	...
	enum IMGSENSOR_HW_POWER_STATUS pwr_status)
{
	int ret = 0;
	enum IMGSENSOR_SENSOR_IDX sensor_idx;
	char *curr_sensor_name = NULL;
	char str_index[LENGTH_FOR_SNPRINTF];

	if (!psensor || !phw) {
		pr_err("[%s] Invalid psensor or phw pointer!\n", __func__);
		return IMGSENSOR_RETURN_ERROR;
	}

	sensor_idx = psensor->inst.sensor_idx;

	if (psensor->inst.psensor_list) {
		curr_sensor_name = psensor->inst.psensor_list->name;
	}

	pr_info("sensor_idx %d, power %d curr_sensor_name %s, enable list %s\n",
			sensor_idx,
		 pwr_status,
		 curr_sensor_name ? curr_sensor_name : "NULL",
