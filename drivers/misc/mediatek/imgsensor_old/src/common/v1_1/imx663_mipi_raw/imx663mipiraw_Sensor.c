...
#define PFX "IMX663_camera_sensor"
/****************************   Modify end	**************************/
#define LOG_INF(format, args...) pr_err(PFX "[%s] " format, __func__, ##args)


BYTE imx663_LRC_data[352] = {0};
UINT16 proc_pdaf_sensor_mode = 3;

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX663_SENSOR_ID,
...
extern int MAIN2_663_otp_read(void);
extern otp_error_code_t IMX663_OTP_ERROR_CODE;
MUINT32  sn_inf_main_imx663[13];  /*0 flag   1-12 data*/
MUINT32  material_inf_main_imx663[4];
MUINT32  af_calib_inf_main_imx663[6];
...
static void imx663_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void imx663_set_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	table_write_cmos_sensor(regDa, regNum*2);
}

#if 0
static void imx663_apply_LRC(void)
{
	...
	memcpy((void *)&puSendCmd[74], imx663_LRC_data+70, 70);
	iBurstWriteReg_multi(puSendCmd, 72, imgsensor.i2c_write_id, 72,
		imgsensor_info.i2c_speed);
	...
	kal_uint32 shutter_2ndframe, kal_uint32 gain_2ndframe)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
	{
		...
		backup_gain_ms = imx663_preview_setting[78];
		backup_gain_ls = imx663_preview_setting[79];
		backup_exposure_ms = imx663_preview_setting[69];
		backup_exposure_ls = imx663_preview_setting[70];

		if (gain != 0) {
			imx663_preview_setting[78] = (gain >> 8) & 0xff;
			imx663_preview_setting[79] = gain & 0xff;
		}
		if (shutter != 0) {
			imx663_preview_setting[69] = (shutter >> 8) & 0xff;
			imx663_preview_setting[70] = shutter & 0xff;
		}

		write_cmos_sensor_16_8(0x0104, 0x01);
		table_write_cmos_sensor(imx663_preview_setting,
				sizeof(imx663_preview_setting) / sizeof(kal_uint16));
		write_cmos_sensor_16_8(0x0104, 0x00);

		imx663_preview_setting[78] = backup_gain_ms;
		imx663_preview_setting[79] = backup_gain_ls;
		imx663_preview_setting[69] = backup_exposure_ms;
		imx663_preview_setting[70] = backup_exposure_ls;
	}
		break;
	case MSDK_SCENARIO_ID_CUSTOM1:
	{
		...
		backup_gain_ms = imx663_custom1_setting[78];
		backup_gain_ls = imx663_custom1_setting[79];
		backup_exposure_ms = imx663_custom1_setting[69];
		backup_exposure_ls = imx663_custom1_setting[70];

		if (gain != 0) {
			imx663_custom1_setting[78] = (gain >> 8) & 0xff;
			imx663_custom1_setting[79] = gain & 0xff;
		}
		if (shutter != 0) {
			imx663_custom1_setting[69] = (shutter >> 8) & 0xff;
			imx663_custom1_setting[70] = shutter & 0xff;
		}

		write_cmos_sensor_16_8(0x0104, 0x01);
		table_write_cmos_sensor(imx663_custom1_setting,
				sizeof(imx663_custom1_setting) / sizeof(kal_uint16));
		write_cmos_sensor_16_8(0x0104, 0x00);

		imx663_custom1_setting[78] = backup_gain_ms;
		imx663_custom1_setting[79] = backup_gain_ls;
		imx663_custom1_setting[69] = backup_exposure_ms;
		imx663_custom1_setting[70] = backup_exposure_ls;
	}
		break;
	case MSDK_SCENARIO_ID_CUSTOM2:
	{
		...
		backup_gain_ms = imx663_custom2_setting[78];
		backup_gain_ls = imx663_custom2_setting[79];
		backup_exposure_ms = imx663_custom2_setting[69];
		backup_exposure_ls = imx663_custom2_setting[70];

		if (gain != 0) {
			imx663_custom2_setting[78] = (gain >> 8) & 0xff;
			imx663_custom2_setting[79] = gain & 0xff;
		}
		if (shutter != 0) {
			imx663_custom2_setting[69] = (shutter >> 8) & 0xff;
			imx663_custom2_setting[70] = shutter & 0xff;
		}

		write_cmos_sensor_16_8(0x0104, 0x01);
		table_write_cmos_sensor(imx663_custom2_setting,
				sizeof(imx663_custom2_setting) / sizeof(kal_uint16));
		write_cmos_sensor_16_8(0x0104, 0x00);

		imx663_custom2_setting[78] = backup_gain_ms;
		imx663_custom2_setting[79] = backup_gain_ls;
		imx663_custom2_setting[69] = backup_exposure_ms;
		imx663_custom2_setting[70] = backup_exposure_ls;
	}
		break;
	case MSDK_SCENARIO_ID_CUSTOM3:
	{
		...
		backup_gain_ms = imx663_custom3_setting[78];
		backup_gain_ls = imx663_custom3_setting[79];
		backup_exposure_ms = imx663_custom3_setting[69];
		backup_exposure_ls = imx663_custom3_setting[70];

		if (gain != 0) {
			imx663_custom3_setting[78] = (gain >> 8) & 0xff;
			imx663_custom3_setting[79] = gain & 0xff;
		}
		if (shutter != 0) {
			imx663_custom3_setting[69] = (shutter >> 8) & 0xff;
			imx663_custom3_setting[70] = shutter & 0xff;
		}
		write_cmos_sensor_16_8(0x0104, 0x01);
		table_write_cmos_sensor(imx663_custom3_setting,
				sizeof(imx663_custom3_setting) / sizeof(kal_uint16));
		write_cmos_sensor_16_8(0x0104, 0x00);

		imx663_custom3_setting[78] = backup_gain_ms;
		imx663_custom3_setting[79] = backup_gain_ls;
		imx663_custom3_setting[69] = backup_exposure_ms;
		imx663_custom3_setting[70] = backup_exposure_ls;
	}
		break;
	case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
	{
		...
		backup_gain_ms = imx663_normal_video_setting[78];
		backup_gain_ls = imx663_normal_video_setting[79];
		backup_exposure_ms = imx663_normal_video_setting[69];
		backup_exposure_ls = imx663_normal_video_setting[70];

		if (gain != 0) {
			imx663_normal_video_setting[78] = (gain >> 8) & 0xff;
			imx663_normal_video_setting[79] = gain & 0xff;
		}
		if (shutter != 0) {
			imx663_normal_video_setting[69] = (shutter >> 8) & 0xff;
			imx663_normal_video_setting[70] = shutter & 0xff;
		}

		write_cmos_sensor_16_8(0x0104, 0x01);
		table_write_cmos_sensor(imx663_normal_video_setting,
				sizeof(imx663_normal_video_setting) / sizeof(kal_uint16));
		write_cmos_sensor_16_8(0x0104, 0x00);

		imx663_normal_video_setting[78] = backup_gain_ms;
		imx663_normal_video_setting[79] = backup_gain_ls;
		imx663_normal_video_setting[69] = backup_exposure_ms;
		imx663_normal_video_setting[70] = backup_exposure_ls;
	}
		break;
	default:
	{
...
static kal_uint32 imx663_ana_gain_table[] = {
	100000,
	100098,
	100196,
	100294,
...
//	uint32_t *pAeCtrls;
	...
	case SENSOR_FEATURE_GET_ANA_GAIN_TABLE:
		if ((void *)(uintptr_t) (*(feature_data + 1)) == NULL) {
			*(feature_data + 0) =
				sizeof(imx663_ana_gain_table);
		} else {
			memcpy((void *)(uintptr_t) (*(feature_data + 1)),
			(void *)imx663_ana_gain_table,
			sizeof(imx663_ana_gain_table));
		}
		break;
	...
	*feature_para_len = 4;
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
		imx663_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		 ...
		 *	(*feature_para_len));
		 */
		imx663_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32)
					   , feature_data_16);
		break;
	...
	case SENSOR_FEATURE_GET_CUSTOM_INFO:
		...
		switch (*feature_data) {
			...
			if (*feature_para_len >= sizeof(MUINT32)*13 + 2) {
			    *(MUINT32 *)(uintptr_t)(*(feature_data+1)) = IMX663_OTP_ERROR_CODE;//otp_state
				memcpy( feature_data+2, sn_inf_main_imx663, sizeof(MUINT32)*13); 
				memcpy( feature_data+10, material_inf_main_imx663, sizeof(MUINT32)*4); 
				memcpy( feature_data+12, af_calib_inf_main_imx663, sizeof(MUINT32)*6);
				#if 0
						for (i = 0 ; i<12 ; i++ ){
						printk("sn_inf_main_imx663[%d]= 0x%x\n", i, sn_inf_main_imx663[i]);
						}
				#endif
			}
				break;
...
UINT32 IMX663_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
} /* IMX663_MIPI_RAW_SensorInit */
