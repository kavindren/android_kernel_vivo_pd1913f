...
/*
 * AK7377AF voice coil motor driver
 *
 *
 */
...
#define DW9781_I2C_SLAVE_ADDR 0xE8
...
int s4AF_WriteReg_RUMBAS4SW(unsigned short i2c_id, unsigned char *a_pSendData,
...
int s4AF_ReadReg_RUMBAS4SW(unsigned short i2c_id, unsigned char *a_pSendData,
			   unsigned short a_sizeSendData,
...
void RUMBAS4SW_WR_I2C(unsigned char slvadr, unsigned char size, unsigned char *dat)
{
	s4AF_WriteReg_RUMBAS4SW(slvadr, dat, size);
}

unsigned short int RUMBAS4SW_RD_I2C(unsigned char slvadr, unsigned char size,unsigned char *dat)
{
	unsigned short int read_data = 0;
	unsigned short int read_data_h = 0;

	s4AF_ReadReg_RUMBAS4SW(slvadr, dat, 2,(unsigned char *)&read_data, 2);
...
void RUMBAS4SW_I2C_OIS_per_write(unsigned short int u16_adr, unsigned short int u16_dat)
{
	...
	if (ret < 0) {
		LOG_INF("i2c transfer fail(%d)", ret);
		ret = -EIO;
	}
	LOG_INF("I2CR09(%d) : 0x%04X  %04X\n", g_pstAF_I2Cclient->addr, u16_adr, out[2]);

	
}

unsigned short int RUMBAS4SW_I2C_OIS_per_read(unsigned short int u16_adr)
{
	...
	return RUMBAS4SW_RD_I2C(0x48, 2, u08_dat);
}
...
static inline int AK7377AF_setVCMPos(unsigned long a_u4Position)
{
	int i4RetValue = 0;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (AK7377AF_setVCMPos(a_u4Position) == 0) {
