...
static int pd2133_main_sub2_main2_count;
...
static struct stAF_DrvList g_stAF_DrvList[MAX_NUM_OF_LENS] = {
	...
	{1, AFDRV_AK7377AFPD2133, AK7377AFPD2133_SetI2Cclient, AK7377AFPD2133_Ioctl,//xyw add pd2133 af
	 AK7377AFPD2133_Release, AK7377AFPD2133_GetFileName, NULL},
...
static DEFINE_MUTEX(af_pinctrl_cnt_mutex);

static int af_pinctrl_init(struct device *pdev)
{
	int ret = 0;

	LOG_INF("af_pinctrl_init start\n");
	/* get pinctrl */
	if (af_pinctrl == NULL) {
		LOG_INF("af_pinctrl is NULL\n");
...
static int af_pinctrl_set(int pin, int state)
{
	...
	case AF_PINCTRL_PIN_HWEN:
		...
		if (state == AF_PINCTRL_PINSTATE_LOW &&
				!IS_ERR(af_hwen_low))
			{
				ret = pinctrl_select_state(af_pinctrl, af_hwen_low);
				if(ret < 0){
					LOG_INF("set pinctrl is LOW fail\n");
				}
			}
		else if (state == AF_PINCTRL_PINSTATE_HIGH &&
				!IS_ERR(af_hwen_high))
			{
				ret = pinctrl_select_state(af_pinctrl, af_hwen_high);
				if(ret < 0){
					LOG_INF("set pinctrl is HIGH fail\n");
				}
			}
		else
...
int set_pinctrl_af_high(void)
{
	int ret = 0;
	
	mutex_lock(&af_pinctrl_cnt_mutex);
	if(0 == pd2133_main_sub2_main2_count){
		ret = af_pinctrl_set(AF_PINCTRL_PIN_HWEN, AF_PINCTRL_PINSTATE_HIGH);
	}
	pd2133_main_sub2_main2_count++;
	mutex_unlock(&af_pinctrl_cnt_mutex);
	
	return ret;
}

int set_pinctrl_af_low(void)
{
	int ret = 0;
	...
	pd2133_main_sub2_main2_count--;
	if(0 == pd2133_main_sub2_main2_count){
		ret = af_pinctrl_set(AF_PINCTRL_PIN_HWEN, AF_PINCTRL_PINSTATE_LOW);
		LOG_INF("PD2133 close af\n");
	}else if(0 > pd2133_main_sub2_main2_count){
		LOG_INF("pd2133_main_sub2_main2_count (%d) error\n", pd2133_main_sub2_main2_count);
		pd2133_main_sub2_main2_count = 0;
	}else{
		LOG_INF("pd2133_main_sub2_main2_count(%d)\n", pd2133_main_sub2_main2_count);
	}
	mutex_unlock(&af_pinctrl_cnt_mutex);
	...
	if (Stage == 0) {
		if (regVCAMAF == NULL) {
			...
			if (node) {
				...
				//FIXME
				if(IS_ERR(regVCAMAF)) {
					regVCAMAF = regulator_get(lens_device, "vcamaf_b");
				}
		...
		if (regVCAMAF != NULL && g_regVCAMAFEn == 0) {
			...
			LOG_INF("regulator_is_enabled1 %d\n", Status);
		...
		if (regVCAMAF != NULL && g_regVCAMAFEn == 1) {
			...
			LOG_INF("regulator_is_enabled2 %d\n", Status);
#ifdef CONFIG_MTK_CAM_PD1934F_EX_G80
		     ...
		     unsigned long a_u4Param)
{
	...
	case AFIOC_S_SETPOWERCTRL:
		if(NULL != ccm_board_version){
			LOG_INF("ccm_board_version = %s  ccm_board_version[0] = %c\n  ", ccm_board_version, ccm_board_version[0]);
			...
			LOG_INF("ccm_board_version is NULL \n");
		}
	break;
#else
	case AFIOC_S_SETPOWERCTRL:
	...
	default:
	/*chenhan modified*/
		if (g_pstAF_CurDrv && g_pstAF_CurDrv->pAF_Ioctl) {
			i4RetValue = g_pstAF_CurDrv->pAF_Ioctl(a_pstFile, a_u4Command, a_u4Param);
		}
		i4RetValue = ois_interface_dispatcher(a_u4Command, (__user void *)(a_u4Param));
		break;
	}
	/*modified end*/
...
static int AF_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	g_s4AF_Opened = 1;
	spin_unlock(&g_AF_SpinLock);
#if defined(CONFIG_MTK_CAM_PD2135)   //pinctrl
	if(NULL != ccm_board_version){
		LOG_INF("ccm_board_version = %s  ccm_board_version[0] = %c\n  ", ccm_board_version, ccm_board_version[0]);
		if('0' != ccm_board_version[0]){	
			set_pinctrl_af_high();
		...
		LOG_INF("ccm_board_version is NULL, pd2133pd2135 power fail\n");
	}
#else   //regulator
	...
	ois_interface_create(g_pstAF_I2Cclient, lens_device, OISDRV_DW9781C, 0,0);
#elif defined(CONFIG_MTK_CAM_PD2085) || defined(CONFIG_MTK_CAM_PD2120A)
	ois_interface_create(g_pstAF_I2Cclient, lens_device, OISDRV_RUMBAS4SW, 0,0);
#endif
...
static int AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if(NULL != ccm_board_version){
		...
		LOG_INF("ccm_board_version id NULL\n");
	}
#else
	af_pinctrl_set(AF_PINCTRL_PIN_HWEN,
	...
	ois_interface_dispatcher(AFIOC_X_OIS_DEINIT, NULL);
#if defined(CONFIG_MTK_CAM_PD2072) || defined(CONFIG_MTK_CAM_PD2085) || defined(CONFIG_MTK_CAM_PD2120A) || defined(CONFIG_MTK_CAM_PD2083F_EX)
	...
	if(NULL != ccm_board_version){
		...
		if('0' != ccm_board_version[0]){	
			if (af_pinctrl_init(&pdev->dev)){
				LOG_INF("%s Failed to init pinctrl.\n", __FILE__);
			}else{
				LOG_INF("%s init pinctrl success.\n", __FILE__);
			}
		}
	}else{
		LOG_INF("ccm_board_version is NULL\n");
