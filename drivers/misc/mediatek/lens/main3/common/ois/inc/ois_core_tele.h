...
struct ois_circlemode_parameter { //control params from user space
	...
	s16 count;
	s16 errCount;
	s16 frequency; // default 2Hz
	s16 angleLimit; //30 --> 3degree
	s16 amplitudeX; //0-100
	s16 amplitudeY;
	s16 checkResult;
};
...
int ois_interface_create_tele(struct i2c_client *client, struct device *pdev, const char *ois_name);
int ois_interface_destroy_tele(void);
long ois_interface_dispatcher_tele(unsigned int ioc_command, __user void *buf);

/*ois util fucs define*/
int ois_i2c_write_tele(struct i2c_client *client, u16 addr, u16 data);
int ois_i2c_read_tele(struct i2c_client *client, u16 addr, u16 *data);
int ois_i2c_write_block_tele(struct i2c_client *client, u16 addr, const u8 *data, size_t size);
int ois_i2c_read_block_tele(struct i2c_client *client, u16 addr, u8 *data, size_t size);
