...
/*ois log define*/
int log_ois_level_tele;
//module_param(log_ois_level_tele, int, 0644);
EXPORT_SYMBOL_GPL(log_ois_level_tele);

static struct ois *ois_instance_tele = NULL;

int ois_interface_create_tele(struct i2c_client *client, struct device *dev, const char *ois_name)
{
	int ret = 0;
	log_ois_level_tele = OIS_LOG_INFO;
	OIS_BUG(!client);
	OIS_BUG(!ois_name);
	LOG_OIS_INF("ois name = %s", ois_name);

	if (ois_instance_tele) {
		LOG_OIS_INF("ois create already done");
		goto p_err;
	}

	ois_instance_tele = (struct ois*)(kzalloc(sizeof(struct ois), GFP_KERNEL));
	if (!ois_instance_tele) {
		LOG_OIS_ERR("ois create fail");
		goto p_err;
	}

	mutex_init(&ois_instance_tele->op_lock);
	mutex_init(&ois_instance_tele->init_lock);
	ois_instance_tele->dev = dev;

	ois_instance_tele->client = (struct i2c_client*)(kzalloc(sizeof(struct i2c_client), GFP_KERNEL));
	if (!ois_instance_tele->client) {
		LOG_OIS_ERR("ois client buffer alloc fail");
		goto p_err_client;
	}
	memcpy((void*)ois_instance_tele->client, (void*)client, sizeof(struct i2c_client));

	if (!strncmp(ois_name, "DW9786", 6)) {
#if defined(CONFIG_MTK_CAM_PD2135)
		dw9786_get_ops(ois_instance_tele);
#endif
	} else {
	...
	ois_instance_tele->flash_info = (struct ois_flash_info *)(kzalloc(sizeof(struct ois_flash_info), GFP_KERNEL));
	if (!ois_instance_tele->flash_info) {
		LOG_OIS_ERR("ois_status_check_buf kzalloc failed(%d)\n", ret);
		ois_instance_tele->flash_info = NULL;
		goto p_err_flash;
	}

	//alloc otp info buffer
	ois_instance_tele->otp_info = (struct ois_otp_info *)(kzalloc(sizeof(struct ois_otp_info), GFP_KERNEL));
	if (!ois_instance_tele->otp_info) {
		LOG_OIS_ERR("ois otp data kzalloc failed(%d)\n", ret);
		goto p_err_otp;
	}

	//alloc o+e info buffer
	ois_instance_tele->lens_info_buf = kzalloc(sizeof(struct ois_lens_info_buf), GFP_KERNEL);
	if (!ois_instance_tele->lens_info_buf) {
		LOG_OIS_ERR("alloc o2e buffer fail");
		goto p_err_o2e;
	};

	LOG_OIS_INF("ois(%s instance=%p client=%p lens_info=%p) create success",
		ois_name, ois_instance_tele, ois_instance_tele->client, ois_instance_tele->lens_info_buf);

	return ret;

p_err_o2e:
	if (ois_instance_tele->otp_info) {
		LOG_OIS_INF("release ois otp(%p)", ois_instance_tele->otp_info);
		kfree(ois_instance_tele->flash_info);
		ois_instance_tele->flash_info = NULL;
	}
p_err_otp:
	if (ois_instance_tele->flash_info) {
		LOG_OIS_INF("release ois flash(%p)", ois_instance_tele->flash_info);
		kfree(ois_instance_tele->flash_info);
		ois_instance_tele->flash_info = NULL;
	}
p_err_flash:
p_err_ops:
	if (ois_instance_tele->client) {
		LOG_OIS_INF("release ois client(%p)", ois_instance_tele->client);
		kfree(ois_instance_tele->client);
		ois_instance_tele->client = NULL;
	}
p_err_client:
	if (ois_instance_tele) {
		LOG_OIS_INF("client fail, release ois buffer(%p)", ois_instance_tele);
		kfree(ois_instance_tele);
		ois_instance_tele = NULL;
	}
p_err:
	return ret;
}

int ois_interface_destroy_tele()
{
	int ret = 0;

	if (ois_instance_tele) {
		if (ois_instance_tele->flash_info) {
			LOG_OIS_INF("release ois flash info buffer(%p)", ois_instance_tele->flash_info);
			kfree(ois_instance_tele->flash_info);
			ois_instance_tele->flash_info = NULL;
		}
		if (ois_instance_tele->lens_info_buf) {
			LOG_OIS_INF("release oe buffer(%p)", ois_instance_tele->lens_info_buf);
			kfree(ois_instance_tele->lens_info_buf);
			ois_instance_tele->lens_info_buf = NULL;
		}
		if (ois_instance_tele->client) {
			LOG_OIS_INF("release ois client buffer(%p)", ois_instance_tele->client);
			kfree(ois_instance_tele->client);
			ois_instance_tele->client = NULL;
		}

		LOG_OIS_INF("release ois(%p)", ois_instance_tele);
		kfree(ois_instance_tele);
		ois_instance_tele = NULL;
	} else {
...
static long ois_dispatcher_tele(struct ois *ois, unsigned int ioc_command, __user void *buf)
{
	long ret = 0;
	int mode = 0x0000;
...
long ois_interface_dispatcher_tele(unsigned int ioc_command, void *buf)
{
	return ois_dispatcher_tele(ois_instance_tele, ioc_command, buf);
}