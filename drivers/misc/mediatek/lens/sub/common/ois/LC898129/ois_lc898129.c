...
static int lc898129_log_control(struct ois *ois, int level)
{
	int ret = 0;
...
static int RamRead32A_CheckIICTimeout(struct ois *ois, UINT_16 addr, UINT_32 *ReadVal)
{
	...
	re  = ois_i2c_read_32_sub(client, addr, ReadVal);
...
static int RamRead32A(struct ois *ois, UINT_16 addr, UINT_32 *ReadVal)
{
	...
	struct i2c_client *client = ois->client;
	OIS_BUG(!client);
	ois_i2c_read_32_sub(client, addr, ReadVal);
	dataTemp = *ReadVal;
	*ReadVal = ((dataTemp & 0xFF) << 24) | (((dataTemp >> 8) & 0xFF) << 16) |
		(((dataTemp >> 16) & 0xFF) << 8) | ((dataTemp >> 24) & 0xFF);
	return re;
...
static int RamWrite32A(struct ois *ois, UINT_16 addr, UINT_32 WriteVal)
{
	...
	struct i2c_client *client = ois->client;
	OIS_BUG(!client);
	re = ois_i2c_write_32_sub(client, addr, WriteVal);
	I2COP_CHECK(re);
p_err:
	return re;
}
...
{
	RamWrite32A(ois, LC898129_REG_OIS_CMD_IO_ADDR, IOadrs);
	RamRead32A(ois, LC898129_REG_OIS_CMD_IO_DATA, IOdata);
}

static void IOWrite32A(struct ois *ois, UINT_32 IOadrs, UINT_32 IOdata)
{
	RamWrite32A(ois, LC898129_REG_OIS_CMD_IO_ADDR, IOadrs);
	RamWrite32A(ois, LC898129_REG_OIS_CMD_IO_DATA, IOdata);
}
...
static int CntWrt(struct ois *ois, UINT_8 *writeval, UINT_16 UcLength)
{
	...
	ret = ois_i2c_write_block_sub(client, addr, WriteValtemp, writeDataSize);
	return ret;
}
...
static int CntRd(struct ois *ois, UINT_32 addr, UINT_8 *ReadVal, UINT_16 UcLength)
{
	...
	struct i2c_client *client = ois->client;
	OIS_BUG(!client);
	ret = ois_i2c_read_block_sub(client, addr, readValtemp, readSize);
...
static UINT_8 RdStatus(struct ois *ois, UINT_8 UcStBitChk)
{
	...
	RamRead32A(ois, LC898129_REG_OIS_STATUS_READ, &UlReadVal);
	if (UcStBitChk) {
...
static int lc898129_init(struct ois *ois)
{
	int ret = 0;
	struct i2c_client *client = ois->client;
	...
	ois->client->addr = LC898129_SLAVE_ADDR >> 1;

	//check if need preprocess of another ic
	if (!ois->slave_mode && NULL != ois->dependency) {
		ret = CALL_OISOPS(((struct ois *)(ois->dependency)), ois_init);
		LOG_OIS_INF("init pre instance %p", ois->dependency);
	}

	//ois init on
	ret = RamWrite32A(ois, LC898129_REG_OIS_SPI_INIT, SPI_6DSOQ_MASTER);
	I2COP_CHECK(ret);
	// ret = RamWrite32A(ois, LC898129_REG_SERVE_CTRL, OIS_SERVE_ON);
	// I2COP_CHECK(ret);
	...
	/////////////////////////////////////////////////////////////
	ret = RamWrite32A(ois, LC898129_REG_OIS_CTRL, OIS_ACC_ON);
	I2COP_CHECK(ret);
	ret = RamWrite32A(ois, LC898129_REG_OIS_ACC_CTRL, OIS_ACC_ON);
	I2COP_CHECK(ret);
	ret = RamRead32A(ois, LC898129_REG_OIS_FW_VERSION_HIGH, &old_version_h);
	ret = RamRead32A(ois, LC898129_REG_OIS_FW_VERSION_LOW, &old_version_l);
	...
	LOG_OIS_INF("X oldh:0x%08x oldl:0x%08x ,newh:0x%08x newl:0x%08x",
...
static int lc898129_deinit(struct ois *ois)
{
	int ret = 0;
	UINT_8 UcStRd = 1;
	UINT_32 UlStCnt = 0;
	...
	/*2.servo off*/
	ret = RamWrite32A(ois, LC898129_REG_OIS_ACC_CTRL, OIS_ACC_OFF);
	I2COP_CHECK(ret);
	// ret = RamWrite32A(ois, LC898129_REG_SERVE_CTRL, OIS_SERVE_OFF);
	// I2COP_CHECK(ret);
	while (UcStRd && (UlStCnt++ < CNT050MS)) {
		UcStRd = RdStatus(ois, 1);
	}
	ret = RamWrite32A(ois, LC898129_REG_OIS_CTRL, OIS_ACC_OFF);
	I2COP_CHECK(ret);
...
// In development
static int lc898129_stream_on(struct ois *ois)
{
	int ret = 0;
	return ret;
}

static int lc898129_stream_off(struct ois *ois)
{
	int ret = 0;
	return ret;
}
...
// get ois mode
static int lc898129_get_mode(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
...
static int lc898129_set_mode(struct ois *ois, int mode)
{
	int ret = 0;
	int user_mode = 0;
	...
	case OIS_STILL_MODE: {
		// exp_mode = LC898129_STILL_MODE;
		exp_mode = OIS_ACC_ON;
		ret = RamWrite32A(ois, LC898129_REG_OIS_MOOD, OIS_MODE_STILL);
		I2COP_CHECK(ret);
		ret = RamWrite32A(ois, LC898129_REG_OIS_CTRL, OIS_ACC_ON);
		I2COP_CHECK(ret);
		ret = RamWrite32A(ois, LC898129_REG_OIS_ACC_CTRL, OIS_ACC_ON);
	...
	case OIS_VIDEO_MODE: {
		...
		ret = RamWrite32A(ois, LC898129_REG_OIS_MOOD, OIS_MODE_MOVIE);
		I2COP_CHECK(ret);
		ret = RamWrite32A(ois, LC898129_REG_OIS_CTRL, OIS_ACC_ON);
		I2COP_CHECK(ret);
		ret = RamWrite32A(ois, LC898129_REG_OIS_ACC_CTRL, OIS_ACC_ON);
	...
	if (old_mode != user_mode) {
		ret = ois_i2c_write(client, LC898129_REG_OIS_MODE, exp_mode);
		I2COP_CHECK(ret);
		ret = ois_i2c_write(client, LC898129_REG_OIS_CMD_STATUS, OPERATE_START);
		I2COP_CHECK(ret);
	}
	*/
	ret = RamRead32A(ois, LC898129_REG_OIS_MOOD, &new_mode);
	// ret = ois_i2c_read(client, LC898129_REG_OIS_CMD_STATUS, &op_status);
...
static UINT_8 UnlockCodeSet(struct ois *ois)
{
	...
	do {
		IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_UNLK_CODE1, OIS_UNLOCK_CODE_1);
		IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_UNLK_CODE2, OIS_UNLOCK_CODE_2);
		IORead32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAWP, &UlReadVal);
...
static UINT_8 UnlockCodeClear(struct ois *ois)
{
	...
	do {
		IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAWP, 0x00000010);
		IORead32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAWP, &UlDataVal);
...
{
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_UNLK_CODE3, 0x0000ACD5);
}

static void WritePermission(struct ois *ois)
{
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_RSTB_FLA, LC898129_REG_ON);
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_CLK_FLAON, 0x00000010);
}
// Erase User Mat
static UINT_8 EraseUserMat129(struct ois *ois, UINT_8 StartBlock, UINT_8 EndBlock)
{
	...
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAMODE, 0x00000000);
	RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_SETUP, 0x00000000);

	for (i = StartBlock; i < EndBlock; i++) {
		RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_STARADDR, (i << 10));
		RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_CTL, 0x00000020);
		...
		do {
			mdelay(5);
			if (UlCnt++ > 100) {
				IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAMODE, 0x00000002);
				return 0x31;
			}
			RamRead32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_CTL, &UlReadVal);
		} while (UlReadVal != 0);
	}
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAMODE, 0x00000002);
	return 0;
...
static UINT_8 ProgramFlash129_Standard(struct ois *ois, CODE_TBL_EXT *ptr)
{
	...
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAMODE, 0x00000000);
	RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_SETUP, 0x00000000);
	RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_STARADDR, 0x00000010);
	data[0] = 0xF0;
	data[1] = 0x08;
	data[2] = 0x00;
	...
	for (i = 1; i < (ptr->SizeFromCode / 64); i++) {
		...
		RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_LENS, 0x00000010);
		UlCnt = 0;
		if (UcOddEvn == 0) {
			do {
				mdelay(1);
			...
			RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_CTL, 0x00000004);
		} else {
			do {
				mdelay(1);
				RamRead32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_CTL, &UlReadVal);
			...
			RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_CTL, 0x00000008);
		}
	}
	...
	RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_STARADDR, 0x00000000);
	data[1] = 0x08;
	...
	UINT_32 UlAddress, UINT_32 *PulData, UINT_8 UcLength)
{
	...
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_ACSCNT, 0x00000000 | (UINT_32)(UcLength - 1));
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLA_ADR, ((UINT_32)SelMat << 16) | (UlAddress & 0x00003FFF));
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_CMD, 0x00000001);
	for (i = 0; i < UcLength; i++) {
		IORead32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLA_RDAT, &PulData[i]);
	}
...
static UINT_8 FlashBlockErase129(struct ois *ois, UINT_8 SelMat, UINT_32 SetAddress)
{
	...
	WritePermission(ois);
	if (SelMat == TRIM_MAT) {
		IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_UNLK_CODE3, 0x00005B29);
	} else if (SelMat != USER_MAT) {
		IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_UNLK_CODE3, 0x0000C5AD);
	}
	AdditionalUnlockCodeSet(ois);

	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLA_ADR, ((UINT_32)SelMat << 16) | (SetAddress & 0x00003C00));
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_CMD, 4);
	...
	do {
		...
		IORead32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAINT, &UlReadVal);
...
static UINT_8 FlashPageWrite(struct ois *ois, UINT_8 SelMat, UINT_32 SetAddress, UINT_32 *PulData)
{
	...
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLA_ADR, ((UINT_32)SelMat << 16) | (SetAddress & 0x00003FF0));
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_CMD, 2);

	UlCnt = 0;

	for (i = 0; i < 16; i++) {
		IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLA_WDAT, PulData[i]);
	}
	do {
		if (UlCnt++ > 100) {
			ans = 2;
	...
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_CMD, 8);
	do {
		if (UlCnt++ > 100) {
			ans = 2;
			break;
...
static UINT_8 PmemUpdate129(struct ois *ois, UINT_8 dist, CODE_TBL_EXT *ptr)
{
	...
	// 1.
	//--------------------------------------------------------------------------------
	RamWrite32A(ois, LC898129_REG_OIS_PMEM_STARADDR, 0x00080000);
	...
	CntRd(ois, LC898129_REG_OIS_PMEM_CHECKSUM, ReadData, 8);
	...
	if (dist != 0) {
		RamWrite32A(ois, LC898129_REG_OIS_REMAP, 0);
	}
...
static UINT_8 FlashUpdate129(struct ois *ois, CODE_TBL_EXT *ptr)
{
	...
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAMODE, 0x00000000);
	RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_STARADDR, 0x00000000);
	RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_CHECKSUM, ptr->SizeFromCodeValid);

	RamWrite32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_CTL, 0x00000100);
	mdelay(6);
	UlCnt = 0;
	do {
		RamRead32A(ois, LC898129_REG_OIS_FLASHROM_ACCESS_CTL, &UlReadVal);
	...
	RamRead32A(ois, LC898129_REG_OIS_FLASHROM_CHECKSUM, &UlReadVal);
	...
	IOWrite32A(ois, LC898129_REG_OIS_CMD_SYSDSP_REMAP, 0x00001001);
	mdelay(15);
	IORead32A(ois, LC898129_REG_OIS_CMD_ROMINFO, (UINT_32 *)&UlReadVal);
	if (UlReadVal != 0x0A)
		return 0x53;
...
// update fw
static int lc898129_fw_update(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
	UINT_32 old_version_h = 0;
	UINT_32 old_version_l = 0;
	...
	ret = RamRead32A_CheckIICTimeout(ois, LC898129_REG_OIS_FW_VERSION_HIGH, &old_version_h);
	if (ret) {
	...
	LOG_OIS_INF("FirmwareUpdateLC898129 :%x	%x ,%x	%x",
		old_version_h, old_version_l, exp_version_h, exp_version_l);
	...
	if ((old_version_h != exp_version_h) || (old_version_l != exp_version_l)) {
		...
		do {
			if (ptr->Index == (((UINT_16)ModuleVendor << 8) + ActVer)) {
				// set in boot mode
				IORead32A(ois, LC898129_REG_OIS_CMD_SYSDSP_REMAP, &ReadVal);
				ReadVal = (ReadVal & 0x1) | 0x00001400;
				IOWrite32A(ois, LC898129_REG_OIS_CMD_SYSDSP_REMAP, ReadVal);
				I2COP_CHECK(ret);
				mdelay(15);
				// flash update
...
// get_fw_version
static int lc898129_get_fw_version(struct ois *ois, __user void *user_buf)
{
	int ret = 0;
	struct ois_flash_info *flash_info = ois->flash_info;
...
static int lc898129_get_gyro_offset(struct ois *ois, __user void *user_buf)
{
	int ret = 0;
	struct ois_flash_info *flash_info = ois->flash_info;
...
static void SrvOn(struct ois *ois)
{
	...
	RamWrite32A(ois, LC898129_REG_SERVE_CTRL, OIS_SERVE_ON);
...
{
	if (mode == 0) {
		*measadr_a = LC898129_REG_OIS_GYROCAL_RAM_GX_ADIDAT;
		*measadr_b = LC898129_REG_OIS_GYROCAL_RAM_GY_ADIDAT;
	} else if (mode == 1) {
		*measadr_a = LC898129_REG_OIS_GYROCAL_ZRAM_GZ_ADIDAT;
		*measadr_b = LC898129_REG_OIS_GYROCAL_ACCLRAM_Z_AC_ADIDAT;
	} else {
		*measadr_a = LC898129_REG_OIS_GYROCAL_ACCLRAM_X_AC_ADIDAT;
		*measadr_b = LC898129_REG_OIS_GYROCAL_ACCLRAM_Y_AC_ADIDAT;
	}
}
...
static UINT_32 MeasGyAcOffset129(struct ois *ois)
{
	...
	LOG_OIS_INF("status:%d", SlMeasureAveValueB[1]);
	...
	if (UlRsltSts == OIS_CALNG_EXE_END) {
		RamWrite32A(ois, LC898129_REG_OIS_GYROCAL_RAM_GXOFFZ,
								SlMeasureAveValueA[0]); // X axis Gyro offset
		RamWrite32A(ois, LC898129_REG_OIS_GYROCAL_RAM_GYOFFZ,
								SlMeasureAveValueB[0]); // Y axis Gyro offset
		RamWrite32A(ois, LC898129_REG_OIS_GYROCAL_ZRAM_GZOFFZ,
								SlMeasureAveValueA[1]); // Z axis Gyro offset
		RamWrite32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_X_AC_OFFSET,
								SlMeasureAveValueA[2]); // X axis Accel offset
		RamWrite32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_Y_AC_OFFSET,
								SlMeasureAveValueB[2]); // Y axis Accel offset
		RamWrite32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_Z_AC_OFFSET,
...
static UINT_8 WrGyAcOffsetData129(struct ois *ois, UINT_8 UcMode)
{
	...
	if (UcMode) {
		RamRead32A(ois, LC898129_REG_OIS_GYROCAL_RAM_GXOFFZ, &UlReadGx);
		RamRead32A(ois, LC898129_REG_OIS_GYROCAL_RAM_GYOFFZ, &UlReadGy);
		RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ZRAM_GZ_ADIDAT, &UlReadGz);
		RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_X_AC_OFFSET, &UlReadAx);
		RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_Y_AC_OFFSET, &UlReadAy);
		RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_Z_AC_OFFSET, &UlReadAz);
...
static int GetGyroOffset(struct ois *ois, UINT_16 *GyroOffsetX, UINT_16 *GyroOffsetY, UINT_16 *GyroOffsetZ)
{
	...
	INT_32 re = RamRead32A(ois, LC898129_REG_OIS_GYROCAL_RAM_GXOFFZ, &ReadValX);
	re |= RamRead32A(ois, LC898129_REG_OIS_GYROCAL_RAM_GYOFFZ, &ReadValY);
	re |= RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ZRAM_GZOFFZ, &ReadValZ);
...
{
	UINT_32 ReadValX = 0, ReadValY = 0, ReadValZ = 0;
	int re = RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_X_AC_OFFSET, &ReadValX);
	re |= RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_Y_AC_OFFSET, &ReadValY);
	re |= RamRead32A(ois, LC898129_REG_OIS_GYROCAL_ACCLRAM_Z_AC_OFFSET, &ReadValZ);
...
//
static int lc898129_set_offset_calibration(struct ois *ois)
{
	int ret = 0;
	UINT_8 UcStRd = 1;
	UINT_32 UlStCnt = 0;
	...
	// IOWrite32A(ois, LC898129_REG_OIS_CMD_SYSDSP_REMAP, 0x1000);
	// mdelay(30);
	...
	// ret = RamWrite32A(ois, LC898129_REG_OIS_CTRL, 0x00000001 ) ;
	while (UcStRd && (UlStCnt++ < CNT050MS)) {
		UcStRd = RdStatus(ois, 1);
	}
	mdelay(100);
...
//
static int lc898129_get_gyro_gain(struct ois *ois, __user void *user_buf)
{
	int ret = 0;
	struct ois_flash_info *flash_info = ois->flash_info;
	UINT_32 ReadVal = 0;
...
//
static int lc898129_set_gyro_gain(struct ois *ois, __user void *user_buf)
{
	...
	UINT_32 SetAddress, UINT_32 *PulData)
{
	...
	if (SelMat != USER_MAT) {
		if (SelMat == INF_MAT2)
			IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_UNLK_CODE3, 0x00006A4B);
		else
			IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_UNLK_CODE3, 0x0000C5AD);
	}
	AdditionalUnlockCodeSet(ois);
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLA_ADR, ((UINT_32)SelMat << 16) | (SetAddress & 0x000010));
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_FLAMODE, 0x00000000);
	IOWrite32A(ois, LC898129_REG_OIS_CMD_FLASHROM_CMD, 2);
...
//
static int lc898129_flash_save(struct ois *ois)
{
	...
	if (0 == ret) {
		IOWrite32A(ois, LC898129_REG_OIS_CMD_SYSDSP_REMAP, 0x1000);
		mdelay(30);
		RamWrite32A(ois, LC898129_REG_OIS_SPI_INIT, 0x00000004);
		while (UcStRd && (UlStCnt++ < CNT050MS)) {
			UcStRd = RdStatus(ois, 1);
		}
		...
		RamWrite32A(ois, LC898129_REG_OIS_CTRL, 0x00000003);
		while (UcStRd && (UlStCnt++ < CNT050MS)) {
			UcStRd = RdStatus(ois, 1);
		}
		mdelay(100);
...
// update distance
static int lc898129_set_acc(struct ois *ois, __user void *user_buf)
{
	int ret = 0;
	...
	tx_data = (u32)acc_info.currFocusDistance;
...
// fix
static int lc898129_set_target(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
	struct i2c_client *client = ois->client;
	...
	OIS_BUG(!client);
	OIS_BUG(!user_buf);
	RamWrite32A(ois, LC898129_REG_OIS_ACC_CTRL, OIS_ACC_OFF);
	I2COP_CHECK(ret);
	RamWrite32A(ois, LC898129_REG_OIS_CTRL, OIS_ACC_OFF);
	I2COP_CHECK(ret);
	RamWrite32A(ois, LC898129_REG_SERVE_CTRL, OIS_SERVE_ON);
	I2COP_CHECK(ret);
	...
	switch (fixmode.mannualEnable) {
	case LEFTTOP: {
		fixmode.targetX = ois->otp_info->hallXMax;
		fixmode.targetY = ois->otp_info->hallYMax;
		break;
	}
	case LEFTBOTTOM: {
		fixmode.targetX = ois->otp_info->hallXMin;
		fixmode.targetY = ois->otp_info->hallYMax;
		break;
	}
	case RIGHTTOP: {
		fixmode.targetX = ois->otp_info->hallXMax;
		fixmode.targetY = ois->otp_info->hallYMin;
		break;
	}
	case RIGHTBOTTOM: {
		fixmode.targetX = ois->otp_info->hallXMin;
...
// init data
static int lc898129_get_init_info(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
	struct ois_flash_info *flash_info = ois->flash_info;
	OIS_BUG(!flash_info);
	...
	LOG_OIS_INF("control info: chip(0x%04x) dsp(0x%04x) writeAthr(0x%04x) reset(0x%04x) tripod(0x%04x) mode(0x%04x) acc(0x%04x) readyflag(0x%04x)",
		...
		flash_info->tripodFlag, flash_info->imuInfo.spiMode, flash_info->accOn, flash_info->chipEn);
p_err:
	return ret;
}

// status check
static int lc898129_status_check(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
...
// otp
static int lc898129_format_otp_data(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
	u8 *otp_buf = NULL;
	u8 *sn_data = NULL;
	...
	u8 data_size = 0;
	u8 idx = 0;
	struct ois_otp_info *ois_otp = ois->otp_info;
	s16 s16_data = 0x0000;
...
static int SetSinWaveParam(struct ois *ois, int frequency, int mode)
{
	...
	if (mode == 255) {
		RamWrite32A(ois, LC898129_REG_OIS_SinWave_Phase, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_CosWave_Phase, 0x20000000);
	} else {
		RamWrite32A(ois, LC898129_REG_OIS_SinWave_Phase, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_CosWave_Phase, 0x00000000);
	}

	if (frequency_t == 0xFFFFFFFF) {
		RamWrite32A(ois, LC898129_REG_OIS_SinWave_Offset, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_SinWave_Phase, 0x00000000);

		RamWrite32A(ois, LC898129_REG_OIS_CosWave_Offset, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_CosWave_Phase, 0x00000000);

		RamWrite32A(ois, LC898129_REG_OIS_SinWaveC_Regsiter, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_SinWave_OutAddr, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_CosWave_OutAddr, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_FIX_TARGET_X, 0x00000000);
		RamWrite32A(ois, LC898129_REG_OIS_FIX_TARGET_Y, 0x00000000);
	} else {
		RamWrite32A(ois, LC898129_REG_OIS_SinWave_Offset, frequency_t);
		RamWrite32A(ois, LC898129_REG_OIS_CosWave_Offset, frequency_t);

		RamWrite32A(ois, LC898129_REG_OIS_SinWaveC_Regsiter, 0x00000001);
		RamWrite32A(ois, LC898129_REG_OIS_SinWave_OutAddr, 0x000000DC);
		RamWrite32A(ois, LC898129_REG_OIS_CosWave_OutAddr, 0x0000012C);
	}
	return 0;
}

// sin mode
static int lc898129_set_sinewave(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
	struct i2c_client *client = ois->client;
	...
	RamRead32A(ois, LC898129_REG_OIS_SinWave_Gain, &amplitudex);
	RamRead32A(ois, LC898129_REG_OIS_CosWave_Gain, &amplitudey);
	if ((amplitudex) || (amplitudey)) {
		LOG_OIS_ERR("LC898129_set_sinewave skip(%d %d %d %d)", sine.axis, sine.frequency, amplitudex, amplitudey);
		return ret;
	}
	// return ret;
	amplitudex = (sine.amplitudeX & 0x0000FFFF) << 16;
	amplitudey = (sine.amplitudeY & 0x0000FFFF) << 16;
	RamWrite32A(ois, LC898129_REG_OIS_SinWave_Gain, amplitudex); // 7fffffff-8000
	RamWrite32A(ois, LC898129_REG_OIS_CosWave_Gain, amplitudey); // 7fffffff-8000
...
// stroke limit
static int lc898129_set_stroke_limit(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
	return ret;
}
...
// set pantilt
static int lc898129_set_pantilt(struct ois *ois, __user void *user_buf)
{
	int ret = 0;
	return ret;
}

static int lc898129_reset(struct ois *ois)
{
	int ret = 0;
	return ret;
}

static int lc898129_set_tripod(struct ois *ois, __user void *user_buf)
{
	int ret = 0;
	return ret;
}
static int lc898129_set_smooth(struct ois *ois, __user void *user_buf)
{
	int ret = 0;
	return ret;
}

static void lc898129_vsync_process_fn(struct kthread_work *work)
{
	int result = 0;
	struct ois *ois = NULL;
	...
	mdelay(3);
	result = ois_i2c_read_block_sub(client, 0xF111, data, 60);
	I2COP_CHECK(result);
	...
	for (group_idx = 0, data_idx = 0; group_idx < 10; group_idx++) {
		lens_info->ic_timecount[group_idx] =
				(s16)((data[data_idx + 1] << 8) | (data[data_idx])); // convert to us
...
static int lc898129_vsync_signal(struct ois *ois, void *buf)
{
	int ret = 0;
	...
	ois->vsync_info.vsync_cnt = tempInfo->vsync_cnt;
	...
	LOG_OIS_INF("isp vsync %llu timestamp %llu",
		ois->vsync_info.vsync_cnt, ois->vsync_info.sof_timestamp_boot);

	return ret;
}

static int lc898129_init_vsync_thread(struct ois *ois)
{
	int ret = 0;
	...
	if (ois->vsync_task == NULL) {
		...
		kthread_init_work(&ois->vsync_work, lc898129_vsync_process_fn);
...
static int lc898129_deinit_vsync_thread(struct ois *ois)
{
	int ret = 0;
	...
	if (!IS_ERR_OR_NULL(ois->vsync_task)) {
		...
		if (ret)
			LOG_OIS_ERR("vsync processor stop fail(%d)", ret);
		ois->vsync_task = NULL;
...
static int lc898129_get_lens_info(struct ois *ois, void __user *user_buf)
{
	int ret = 0;
	//u8 idx = 0;
	struct timespec start_time, end_time;
...
static struct ois_core_ops lc898129_ois_ops = {
	.ois_init = lc898129_init,
	.ois_deinit = lc898129_deinit,
	.ois_stream_on = lc898129_stream_on,
	.ois_stream_off = lc898129_stream_off,
	.ois_get_mode = lc898129_get_mode,
	.ois_set_mode = lc898129_set_mode,
	.ois_fw_update = lc898129_fw_update,
	.ois_get_fw_version = lc898129_get_fw_version,
	.ois_get_gyro_offset = lc898129_get_gyro_offset,
	.ois_set_offset_calibration = lc898129_set_offset_calibration,
	.ois_get_gyro_gain = lc898129_get_gyro_gain,
	.ois_set_gyro_gain = lc898129_set_gyro_gain,
	.ois_flash_save = lc898129_flash_save,
	.ois_set_acc = lc898129_set_acc,
	.ois_set_target = lc898129_set_target,
	.ois_get_init_info = lc898129_get_init_info,
	.ois_status_check = lc898129_status_check,
	.ois_init_vsync_thread = lc898129_init_vsync_thread,
	.ois_deinit_vsync_thread = lc898129_deinit_vsync_thread,
	.ois_vsync_signal = lc898129_vsync_signal,
	.ois_get_lens_info = lc898129_get_lens_info,
	.ois_format_otp_data = lc898129_format_otp_data,
	.ois_set_sinewave = lc898129_set_sinewave,
	.ois_set_stroke_limit = lc898129_set_stroke_limit,
	.ois_set_pantilt = lc898129_set_pantilt,
	.ois_reset = lc898129_reset,
	.ois_set_smooth = lc898129_set_smooth,
	.ois_set_tripod = lc898129_set_tripod,
	.ois_log_control = lc898129_log_control,
};

/*ic entry expose to ois_core*/
void lc898129_get_ops(struct ois *ois)
{
	ois->ops = &lc898129_ois_ops;
}
