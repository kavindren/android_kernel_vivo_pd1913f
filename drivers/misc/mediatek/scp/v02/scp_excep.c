...
void scp_aed(enum SCP_RESET_TYPE type, enum scp_core_id id)
{
	...
	if (!scp_ee_mem_reserved) {
		scp_dump.ramdump = vmalloc(sizeof(struct MemoryDump));
		if (!scp_dump.ramdump) {
			return;
		}

		scp_ee_mem_reserved = 1;
		pr_err("[SCP] scp_ee_mem_reserved= %d\n", scp_ee_mem_reserved);
	}
...
#if 0  // for saving reserved memory.
	scp_dump.ramdump = vmalloc(sizeof(struct MemoryDump));
	if (!scp_dump.ramdump)
		return -1;
#else
	scp_ee_mem_reserved = 0;
#endif
