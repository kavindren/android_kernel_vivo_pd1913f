#ifndef _VSEN_SENSOR_HUB_H_
#define _VSEN_SENSOR_HUB_H_

#ifndef CONFIG_VSEN_SENSOR_CONTEXTHUB
#include <linux/ioctl.h>
#endif

#define VSEN_CMD_DATA_SIZE				10
#define VSEN_USER_SPACE_CMD_DATA_SIZE	(VSEN_CMD_DATA_SIZE+2)

typedef enum{
	SENSOR_COMMAND_COMMON_BOARD_VERSION             = 0x0,
	...
	SENSOR_COMMAND_LIGHT_SET_LCM_ID                 = 0x509,
...
#define VSEN_SENSOR_HUB_SENSOR_CMD _IOW(VSEN_SENSOR_HUB, 0x01, int)


typedef struct SCP_SENSOR_HUB_VSEN_CMD_REQ {
	uint8_t sensorType;
	uint8_t action;
	uint8_t reserve[2];
	uint32_t cmd;
	int32_t data[VSEN_CMD_DATA_SIZE];
} SCP_SENSOR_HUB_VSEN_CMD_REQ;

typedef struct SCP_SENSOR_HUB_VSEN_CMD_RSP {
	uint8_t sensorType;
	uint8_t action;
	uint8_t errCode;
	uint8_t reserve[1];
