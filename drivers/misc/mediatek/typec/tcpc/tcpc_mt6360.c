...
static void usbid_boot_detect_work(struct work_struct *work)
{
	...
	pr_info("%s usbid is present:%d, usbid_present_first_detect: %d \n", __func__,
			chip->usbid_present, chip->usbid_present_first_detect);
}

static void vivo_usbid_detect_work(struct work_struct *work)
{
	int ret = 0;
	struct mt6360_chip *chip = container_of(work, struct mt6360_chip, usbid_detect_work);
...
static int mt6360_pmu_tcpc_irq_register(struct tcpc_device *tcpc)
{
	...
	if (chip->usbid_gpio > 0)
		is_enable = (gpio_get_value(chip->usbid_gpio) == 0);
	else
		is_enable = true;//PD2121 force enable
	pr_info("is_enable = %d\n", is_enable);
#endif
