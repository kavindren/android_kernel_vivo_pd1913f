...
{
	s32 u2_vrt_ref, u2_term_ref, u2_hstx_srctrl, u2_enhance, u2_discth;
	...
	if (!mtk_musb->phy_tuning.inited) {
		...
		mtk_musb->phy_tuning.u2_discth = 0x7;
		...
		if (of_node) {
			...
			of_property_read_u32(of_node,
				"u2_discth", (u32 *) &mtk_musb->phy_tuning.u2_discth);
			of_property_read_u32(of_node,
	...
	u2_discth = mtk_musb->phy_tuning.u2_discth;
	...
	mtk_musb->phy_tuning.u2_discth_real = usb20_phy_read_val(
		OFFSET_RG_USB20_DISCTH, SHFT_RG_USB20_DISCTH, MSK_RG_USB20_DISCTH);

	DBG(0, "%s [real]u2_vrt_ref_real:0x%x  u2_term_ref_real:0x%x  u2_enhance_real:0x%x  u2_hstx_srctrl_real:0x%x, u2_discth_real:0x%x\n",
	    __func__, mtk_musb->phy_tuning.u2_vrt_ref_real, mtk_musb->phy_tuning.u2_term_ref_real,
	    mtk_musb->phy_tuning.u2_enhance_real, mtk_musb->phy_tuning.u2_hstx_srctrl_real, mtk_musb->phy_tuning.u2_discth_real);
	DBG(0, "%s: %d: end\n",  __func__, __LINE__);
}
