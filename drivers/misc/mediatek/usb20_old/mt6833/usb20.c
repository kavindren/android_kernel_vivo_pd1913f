			     ...
			     struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%s: 0x%x\n%s: 0x%x\n%s: 0x%x\n%s: 0x%x\n%s: 0x%x\n",
		       ...
		       "u2_discth", mtk_musb->phy_tuning.u2_discth_real);
}
			      ...
			      const char *buf, size_t count)
{
	...
	*(ptr_val - 1) = 0;
	...
	if (!strcmp(buf_para, "u2_vrt_ref"))
		...
		u2_hstx_srctrl = true;
	else if (!strcmp(buf_para, "u2_discth"))
		u2_discth = true;
	...
	if (u2_vrt_ref) {
		...
		mtk_musb->phy_tuning.u2_discth = val;
		DBG(0, "%s: %d: u2_discth = %d\n", __func__, __LINE__, val);
	}
