...
/*add by sensor team for get pid begin*/
#include <linux/kthread.h>
#include <linux/sched.h>
/*add by sensor team for get pid end*/
...
static void vibrator_enable(unsigned int dur, unsigned int activate)
{
	...
	pr_info(VIB_TAG "pid:%d, cust:%dms, dur:%u, activate:%d, shutdown:%d\n", current->pid,
...
static void vib_shutdown(struct platform_device *pdev)
{
	...
	if (atomic_read(&vibr->vibr_state)) {
		...
		pr_info(VIB_TAG "%s: vibrator will disable\n", __func__);
		vibr_Disable();
	} else {
		spin_unlock_irqrestore(&vibr->vibr_lock, flags);
	}
