...
static int mmc_decode_ext_csd(struct mmc_card *card, u8 *ext_csd)
{
	...
	} else if (card->cid.manfid == CID_MANFID_MICRON) { //add by hezerui for modify device version bit
		card->ext_csd.firmware_version =
				ext_csd[EXT_CSD_FIRMWARE_VERSION + 0] << 0 |
				ext_csd[EXT_CSD_FIRMWARE_VERSION + 1] << 8 |
				ext_csd[EXT_CSD_FIRMWARE_VERSION + 2] << 16 |
	...
	if (card->ext_csd.rev >= 6) {
		...
		card->ext_csd.dev_left_time_b = ext_csd[EXT_CSD_DEVICE_LIFE_TIME_EST_TYP_B];
		card->ext_csd.dev_left_time_a = ext_csd[EXT_CSD_DEVICE_LIFE_TIME_EST_TYP_A];
		card->ext_csd.dev_left_time = ext_csd[EXT_CSD_PRE_EOL_INFO];
	...
	struct mmc_card *oldcard)
{
	...
	if (card->ext_csd.auto_bkops && !card->ext_csd.man_bkops_en) {
		...
		} else if (!card->ext_csd.auto_bkops_en) {
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
