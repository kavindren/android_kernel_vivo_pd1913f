...
"[%s]: msdc%d XXX CMD<%d> resp<0x%.8x>, write protection violation\n",
		...
		if (use_cmd_intr && (mmc_resp_type(cmd) == MMC_RSP_R1B)) {
			 ...
			 */
			if (tmo_type == 0x1 && cmd->opcode != MMC_SLEEP_AWAKE)
	...
	struct mmc_request *mrq)
{
	...
	/* add cmd44 retry in error case */
	do {
		if (msdc_do_cmdq_command(host, cmd, CMD_TIMEOUT))
			cmd44_retry--;
		else
			break;
	} while (cmd44_retry);
	...
	struct mmc_request *mrq, int *retry)
{
	 ...
	 */
	polling_status_tmo = jiffies + 5 * HZ;
	...
	do {
		...
		if (err) {
			...
			(*retry)--;
			if (*retry <= 0) {
				ERR_MSG("get card status always timeout, err = %d", err);
				return 1;
			}
