...
#ifndef CONFIG_NXP_NFC_RETRY
#define CONFIG_NXP_NFC_RETRY
#endif
...
#ifdef CONFIG_NXP_NFC_RETRY
struct nxp_nfc_retry_info {
...
struct nqx_dev {
	...
	struct timespec tv_read;
	struct timespec tv_write;
#ifdef CONFIG_NXP_NFC_RETRY
	struct nxp_nfc_retry_info retry_info;
#endif
};
...
{
	struct nqx_dev *nqx_dev = container_of(work, struct nqx_dev, retry_info.retry_work.work);
	int ret = 0;
	int retry_cnt = 0;

	for (retry_cnt = 1; retry_cnt <= MAX_RETRY_COUNT; retry_cnt++) {
		ret = i2c_master_send(nqx_dev->client, nqx_dev->retry_info.buf, nqx_dev->retry_info.buflen);
		if (ret < 0 || ret != nqx_dev->retry_info.buflen) {
			pr_err("Retry send failed ret: %d\n", ret);
			usleep_range(1000*10, 1000*11);
		} else if (ret == nqx_dev->retry_info.buflen)
			break;
	}
}
#endif
					...
					size_t count, loff_t *offset)
{
	...
	getnstimeofday(&nqx_dev->tv_read);
	/* Read data */
...
#ifdef CONFIG_NXP_NFC_RETRY
	if (!nqx_dev->retry_info.is_read) {
		nqx_dev->retry_info.is_read = true;
		pr_err("%s:cancel_delayed_work retry_work\n", __func__);
		cancel_delayed_work(&nqx_dev->retry_info.retry_work);
	}
#endif
#ifdef NFC_KERNEL_BU
				...
				size_t count, loff_t *offset)
{
	...
	getnstimeofday(&nqx_dev->tv_write);

	if ((nqx_dev->tv_write.tv_sec * 1000 + nqx_dev->tv_write.tv_nsec / 1000000) - (nqx_dev->tv_read.tv_sec * 1000 + nqx_dev->tv_read.tv_nsec / 1000000) > 700) {
...
#ifdef CONFIG_NXP_NFC_RETRY
	if (empty_flag == true) {
		nqx_dev->retry_info.is_read = false;
		nqx_dev->retry_info.buflen = count;
		memcpy(nqx_dev->retry_info.buf, tmp, count);
		pr_err("%s:schedule_delayed_work retry_work\n", __func__);
		schedule_delayed_work(&nqx_dev->retry_info.retry_work, msecs_to_jiffies(1000));
	}
#endif
			...
			const struct i2c_device_id *id)
{
	...
	getnstimeofday(&nqx_dev->tv_write);
#ifdef CONFIG_NXP_NFC_RETRY
	INIT_DELAYED_WORK(&nqx_dev->retry_info.retry_work, retry_wq_handler);
	nqx_dev->retry_info.buf = kzalloc(MAX_BUFFER_SIZE, GFP_KERNEL);
#endif
