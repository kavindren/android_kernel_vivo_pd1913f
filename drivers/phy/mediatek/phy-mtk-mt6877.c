...
{
	s32 u2_vrt_ref, u2_term_ref, u2_enhance, u2_vrt_ref_host, u2_term_ref_host, u2_enhance_host;
	struct device_node *of_node;

	phy_printk(K_ALET, "%s START.\n", __func__);
	if (!instance->phy_tuning.inited) {
		...
		if (of_node) {
			...
			of_property_read_u32(of_node, "u2_vrt_ref_host",
				(u32 *) &instance->phy_tuning.u2_vrt_ref_host);
			of_property_read_u32(of_node, "u2_term_ref_host",
				(u32 *) &instance->phy_tuning.u2_term_ref_host);
			of_property_read_u32(of_node, "u2_enhance_host",
				(u32 *) &instance->phy_tuning.u2_enhance_host);
		}
	...
	u2_vrt_ref_host = instance->phy_tuning.u2_vrt_ref_host;
	u2_term_ref_host = instance->phy_tuning.u2_term_ref_host;
	u2_enhance_host = instance->phy_tuning.u2_enhance_host;

	if (mt_usb_is_host()) {
		phy_printk(K_ALET, "%s [HOST]u2_vrt_ref_host:0x%x u2_term_ref_host:0x%x u2_enhance_host:0x%x\n",
			   __func__, u2_vrt_ref_host, u2_term_ref_host, u2_enhance_host);

		if (u2_vrt_ref_host != -1) {
			if (u2_vrt_ref_host <= VAL_MAX_WIDTH_3) {
				u3phywrite32(U3D_USBPHYACR1,
					     RG_USB20_VRT_VREF_SEL_OFST,
					     RG_USB20_VRT_VREF_SEL, u2_vrt_ref_host);
			}
		}
		if (u2_term_ref_host != -1) {
			if (u2_term_ref_host <= VAL_MAX_WIDTH_3) {
				u3phywrite32(U3D_USBPHYACR1,
					     ...
					     RG_USB20_TERM_VREF_SEL, u2_term_ref_host);
			}
		}
		if (u2_enhance_host != -1) {
			if (u2_enhance_host <= VAL_MAX_WIDTH_2) {
				u3phywrite32(U3D_USBPHYACR6,
					     ...
					     RG_USB20_PHY_REV_6, u2_enhance_host);
			}
		}
	} else {
		phy_printk(K_ALET, "%s [DEVICE]u2_vrt_ref:0x%x u2_term_ref:0x%x u2_enhance:0x%x\n",
	...
	instance->phy_tuning.u2_vrt_ref_real =
		((u3phyread32(U3D_USBPHYACR1) & RG_USB20_VRT_VREF_SEL) >> RG_USB20_VRT_VREF_SEL_OFST) & VAL_MAX_WIDTH_3;
	instance->phy_tuning.u2_term_ref_real =
		((u3phyread32(U3D_USBPHYACR1) & RG_USB20_TERM_VREF_SEL) >> RG_USB20_TERM_VREF_SEL_OFST) & VAL_MAX_WIDTH_3;
	instance->phy_tuning.u2_enhance_real =
		((u3phyread32(U3D_USBPHYACR6) & RG_USB20_PHY_REV_6) >> RG_USB20_PHY_REV_6_OFST) & VAL_MAX_WIDTH_2;
