...
static const struct i2c_device_id bq25890_i2c_id[] = { {"bq25890", 0}, {"bq25890h", 0}, {} };
...
static int bq25890_register_irq(struct bq25890_info *info)
{
	...
	if (!np) {
		pr_err("%s: cannot get of_node\n", __func__);
		return -ENODEV;
	}

	gpio = of_get_named_gpio(np, "vivo,int-gpio", 0);
	if (gpio < 0) {
		pr_err("%s: cannot get vivo,int-gpio (%d)\n", __func__, gpio);
		return -ENODEV;
	}

	ret = gpio_request(gpio, "bq25890_int");
	if (ret < 0)
		pr_err("%s: gpio_request failed (%d)\n", __func__, ret);
	gpio_direction_input(gpio);

	info->irq = gpio_to_irq(gpio);
	pr_info("%s: irq = %d (gpio %d)\n", __func__, info->irq, gpio);
