...
static int fan53870_retry_change_i2c_addr(struct regmap *regmap)
{
	...
	pr_info("fan53870_retry_change_i2c_addr\n");
	u8 fan53870_addr[] = {0x20,0x61,0x72,0x35};

	for(i; i<4 ; i++){
		client->addr = fan53870_addr[i];
		rc = regmap_bulk_read(regmap, reg, &val, 1);
	if (rc < 0) {
		pr_err("failed to write LDO_COMP0 rc=%d,fan53870_addr=%x \n", rc,fan53870_addr[i]);
	}else{
		pr_info("fan53870_retry_change_i2c_addr success");
		client->addr = fan53870_addr[i];
		break;
	}
	}
	...
	retry:
		...
		if (rc < 0) {
			pr_err("failed to read 0x%04x\n", reg);
				...
				if (count_retry > 0 && fan53870_reset() == 0 && fan53870_retry_change_i2c_addr(regmap) ==0)
					goto retry;
		}
	return rc;
}
...
static int fan53870_software_reset(struct regmap *regmap)
{
...
static int fan53870_regulator_is_enabled(struct regulator_dev *rdev)
{
	...
	int rc;
	u8 reg;
	if(gpio_get_value(reset_fan53870.reset_gpio) == 0)
		return 0;
...
static int fan53870_regulator_enable(struct regulator_dev *rdev)
{
	...
	rc = fan53870_read(fan_reg->regmap, 0x13, &reg, 1);
	if (rc < 0) {
		fan53870_err(fan_reg, "failed to read 0x13 rc=%d\n", rc);
	}
	rc = fan53870_read(fan_reg->regmap, 0x14, &reg1, 1);
	if (rc < 0) {
		fan53870_err(fan_reg, "failed to read 0x14 rc=%d\n", rc);
	}
	pr_info("FAN53870 read: 0x13 data:0x%02x, 0x14 data:0x%02x\n", reg, reg1);
				  ...
				  int min_uv, int max_uv)
{
	...
	rc = fan53870_masked_write(fan_reg->regmap, LDO12_SEQ_REG + (fan_reg->offset / 2),
				   0x07 << ((fan_reg->offset & 0x01) * 3), 0);
	if (rc < 0) {
		fan53870_err(fan_reg, "failed to write SEQ_REG rc = %d\n", rc);
		return rc;
	}
			...
			if (chip->init_settings.reg_settings != NULL) {
				for (i = 0; i < num_of_regs; i++) {
					...
					if (chip->init_settings.reg_settings[i].reg_addr == 0x13) {
							...
							chip->init_settings.reg_settings[i].reg_data = 0x96;
						...
						#if defined(CONFIG_MTK_CAM_PD2163)
							chip->init_settings.reg_settings[i].reg_data = 0x15;
				    ...
				    const struct i2c_device_id *id)
{
	...
	rc = fan53870_software_reset(regmap);
	if (rc < 0) {
		pr_err("failed do fan53870_software_reset\n");
		fan53870_cleanGPIO(chip);
		return rc;
	}
