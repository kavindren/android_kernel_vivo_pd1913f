...
#endif
#define DEVICE_DESC_PARAM_WB_SHARED_ALLOC_UNITS 0x55
int ufs_feature_read_unit_desc(struct ufs_hba *hba)
{
	...
	ret = ufsfeature_read_desc(hba, QUERY_DESC_IDN_UNIT, 2,
			selector, /* 0x01 for ufs feature of tw and hpb */
	...
	ufs_ven_lu0_tw_lu_buf_size =
		be32_to_cpu(*(__be32 *)(&unit_buf[UNIT_DESC_TW_LU_MAX_BUF_SIZE]));
...
static int usb_state(struct ufs_hba *hba)
{
	return atomic_read(&hba->usb_state);
}

/* screen state: 1 -> off */
static int screen_state(struct ufs_hba *hba)
{
#if 0
	int state = 0;
...
retry:
	if (!hba->bd) {
		hba->bd = backlight_device_get_by_type(BACKLIGHT_RAW);
		if (hba->bd)
			goto retry;
		return state;
	}

	mutex_lock(&hba->bd->update_lock);
	state = (hba->bd->props.brightness == 0) ? 1 : 0;
	mutex_unlock(&hba->bd->update_lock);

	return state;
#endif
	return !is_panel_backlight_on();
}

static void ufs_usb_status_work(struct work_struct *work)
{
	int rc;
	union power_supply_propval usb_in = {0,};
	struct power_supply *usb_psy;
	struct ufs_hba *hba = container_of(work,
			struct ufs_hba, usb_status_work.work);
	struct ufsf_feature *ufsf = &hba->ufsf;
	unsigned long long t = sched_clock();

	do_div(t, 1000000000); /* seconds */

	/* don't trigger at booting state */
	if (t < 30)
		return;
	if (!ufsf->hid_dev)
		return;
	...
	if (usb_in.intval && !usb_state(hba))
		atomic_set(&hba->usb_state, 1);
	else if (!usb_in.intval && usb_state(hba))
		atomic_set(&hba->usb_state, 0);
	else
		return;

	pr_err("%s usb: %d, lcd: %d, intval: %d\n", __func__, usb_state(hba),
			screen_state(hba), usb_in.intval);

	mutex_lock(&ufsf->hid_dev->sysfs_lock);
	/* Trigger hid when plug usb for charging */
	if (ufshid_get_state(ufsf) == HID_PRESENT) {
		if (usb_state(hba))
			ufshid_trigger_on(ufsf->hid_dev);
		else
			ufshid_trigger_off(ufsf->hid_dev);
	}
	mutex_unlock(&ufsf->hid_dev->sysfs_lock);
}

static int ufs_usb_state_notifier_callback(struct notifier_block *nb,
					       unsigned long event, void *data)
{
	...
	if (strcmp(usb_psy->desc->name, "usb") == 0) {
		hba = container_of(nb, struct ufs_hba, usb_nb);
		schedule_delayed_work(&hba->usb_status_work, 0);
	}

	return NOTIFY_OK;
}

void ufs_init_device_callback(struct ufs_hba *hba)
{
	int ret;
	...
	hba->bd = backlight_device_get_by_type(BACKLIGHT_RAW);
	if (!hba->bd) {
		pr_info("%s backlight device list empty\n", __func__);
	}
#endif
	INIT_DELAYED_WORK(&hba->usb_status_work, ufs_usb_status_work);

	hba->usb_nb.notifier_call = ufs_usb_state_notifier_callback;
	ret = power_supply_reg_notifier(&hba->usb_nb);
	if (ret)
		pr_err("failed to reg power supply notifier: %d\n", ret);
}

void ufs_exit_device_callback(struct ufs_hba *hba)
{
	if (!hba)
		return;

	cancel_delayed_work_sync(&hba->usb_status_work);
	power_supply_unreg_notifier(&hba->usb_nb);
	hba->usb_nb.notifier_call = NULL;
}
#endif
