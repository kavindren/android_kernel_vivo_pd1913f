...
{
	return (ufsf->num_lu == atomic_read(&ufsf->slave_conf_cnt));
}

void ufshpb_init_handler(struct ufsf_feature *ufsf)
{
	int ret;
	...
	if (ret == 0) {
		ERR_MSG("[%s:%d] Probing LU is not fully completed. num_lu=%d, slave_conf_cnt=%d\n",
				__func__, __LINE__, ufsf->num_lu, atomic_read(&ufsf->slave_conf_cnt));
		return;
	}
