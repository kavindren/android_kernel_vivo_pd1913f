...
struct boardinfo_attribute {
	...
	ssize_t (*show)(struct boardinfo_obj *board_info, 
					struct boardinfo_attribute *attr, 
					char *buf);
	ssize_t (*store)(struct boardinfo_obj *board_info, 
					struct boardinfo_attribute *attr, 
...
{
        return boardinfo_ext? boardinfo_ext->project_name:"";
}
EXPORT_SYMBOL_GPL(get_board_name);
...
static int vivo_boardinfo_ext_init(void)
{
	...
	strlcpy(boardinfo_ext->cpu_type, "default", VIVO_CPU_TYPE_LEN);
	strlcpy(boardinfo_ext->vendor, "vivo", VIVO_VENDOR_LEN);
	strlcpy(boardinfo_ext->user_cpu_freq, "default", VIVO_VENDOR_LEN);
	strlcpy(boardinfo_ext->project_name, "default", VIVO_VENDOR_PROJECT_NAME);
...
static const struct of_device_id boardinfo_match_table[] = {
...
MODULE_DESCRIPTION("Hardware Boards Info");
