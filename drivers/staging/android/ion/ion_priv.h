...
#endif
#ifdef CONFIG_RSC_ION_OPT
	bool rsc_reserve_ion_enable;
	struct delayed_work reserve_ion_worker;
#endif
};
...
pool_count_below_lowmark(struct ion_page_pool *pool)
{
	return atomic_read(&pool->count) < get_pool_lowmark(pool);
}

static __always_inline bool
pool_fillmark_reached(struct ion_page_pool *pool)
{
