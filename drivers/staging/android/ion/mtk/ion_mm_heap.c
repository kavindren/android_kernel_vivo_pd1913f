...
#define ION_OPT_SHRINK_POOL_SIZE_MB 10
#define ION_OPT_SLOWPATH_SHRINK_POOL_SIZE_MB 30
#endif

#ifdef CONFIG_RSC_ION_REFILL
#define VALUEABLE_MIN_REFILL_ORDER 4
#endif
...
#ifdef CONFIG_RSC_ION_REFILL
enum ion_page_pool_types {
	ION_UNCACHE_PAGE_POOL,
	ION_CACHE_PAGE_POOL,
	NUM_ION_PAGE_POOLS,
};
#endif
...
#ifdef CONFIG_RSC_ION_OPT
static void ion_sys_heap_reserve(struct work_struct *work);
#endif
...
#ifdef CONFIG_RSC_ION_REFILL
	struct task_struct *kworker[NUM_ION_PAGE_POOLS];
#endif
};
				      ...
				      unsigned long order)
{
	...
	struct ion_page_pool *backend_pool;
	...
	if (!cached) {
		...
		backend_pool = heap->cached_pools[order_to_index(order)];
	} else {
		pool = heap->cached_pools[order_to_index(order)];
		backend_pool = heap->pools[order_to_index(order)];
	}

	page = ion_page_pool_alloc_gather(pool, backend_pool, &from_backend);
#else
	if (!cached)
		pool = heap->pools[order_to_index(order)];
	else
...
#ifdef CONFIG_RSC_ION_REFILL
	if (order >= VALUEABLE_MIN_REFILL_ORDER && pool_count_below_lowmark(pool))
...
struct ion_system_heap *rsc_system_heap;
extern int rsc_ion_page_pool_add(struct ion_page_pool *pool, struct page *page);

u32 rsc_reserve_ion_mode = RSC_RESERVE_ION_WHEN_RAMSIZE_BIGGER_SEVEN_GB;
int reclaim_file_cache_page = RSC_MEM_ION_RECLAIM_FILECACHE_PAGE;
int __read_mostly max_page_pool_size = INT_MAX;
int __read_mostly rsc_system_max_ion_page = RSC_SYSTEM_MAX_ION_POOL_PAGE;
#define RSC_ION_POOL_PERCENT (100)
 ...
 * order 4 200MB
*/
static uint __read_mostly reserve_order_size[2] = {
	100 << (20 - PAGE_SHIFT),
	250 << (20 - PAGE_SHIFT),
};

/* order 8 max size in pool, unit page*/
module_param_named(ion_pool_order8__size, reserve_order_size[0], uint,
		   S_IRUGO | S_IWUSR);

/* order 4 max size in pool, unit page*/
module_param_named(ion_pool_order4__size, reserve_order_size[1], uint,
		   S_IRUGO | S_IWUSR);

/* the max ion pages that keeped in pool*/
module_param_named(system_max_ion_page, rsc_system_max_ion_page, int,
		   S_IRUGO | S_IWUSR);

module_param_named(reclaim_file_cache_page, reclaim_file_cache_page, int,
		   S_IRUGO | S_IWUSR);

module_param_named(spool0, rsc_ion_pool[0], uint, S_IRUGO | S_IWUSR);
module_param_named(spool1, rsc_ion_pool[1], uint, S_IRUGO | S_IWUSR);
module_param_named(spool2, rsc_ion_pool[2], uint, S_IRUGO | S_IWUSR);
module_param_named(spool3, rsc_ion_pool[3], uint, S_IRUGO | S_IWUSR);

#define RSC_MEM_SEVEN_GB_PAGES ((7 * 1024UL) << (20 - PAGE_SHIFT))

/*order 8 size, unit MB*/
static int page_pool_order8 = 100;
/*order 4 size, unit MB*/
static int page_pool_order4 = 250;

module_param(page_pool_order8, int, 0600);
module_param(page_pool_order4, int, 0600);

static void ion_sys_heap_reserve(struct work_struct *work);

static DECLARE_DELAYED_WORK(reserve_ion_worker, ion_sys_heap_reserve);

#define reserve_max_pool_size                                                  \
	(((page_pool_order8 + page_pool_order4) << (20 - PAGE_SHIFT)) *        \
	 (RSC_ION_POOL_PERCENT / 100))

extern int rsc_recovery_survival_mode;

static inline unsigned int ion_sys_heap_charge(struct ion_page_pool *pool,
					       unsigned int page_cnt)
{
	...
	BUG_ON(!pool->cached);
	while ((global_zone_page_state(NR_ION) < reserve_max_pool_size) &&
	       (count < page_cnt)) {
		...
		mod_zone_page_state(page_zone(page), NR_ION,
				    (1 << pool->order));
#endif
		...
		for (i = 0; i < (1 << pool->order); i++)
			SetPageIommu(&page[i]);
		rsc_ion_page_pool_add(pool, page);
		count += 1 << pool->order;
	}
...
static void ion_sys_heap_reserve(struct work_struct *work)
{
	...
	u64 c1, c2;
	int size;
	static atomic_t reserve_time = ATOMIC_INIT(0);
	int set_size[3] = { page_pool_order8, page_pool_order4 };
	struct ion_heap *heap = container_of(
		work, struct ion_heap, reserve_ion_worker.work);
	/*only reserver one time*/
	if (atomic_inc_return(&reserve_time) > 1)
		goto out;

	if ((rsc_reserve_ion_mode & RSC_RESERVE_ION_FORCE) ||
	    ((rsc_reserve_ion_mode &
	      RSC_RESERVE_ION_WHEN_RAMSIZE_BIGGER_SEVEN_GB) &&
	     (totalram_pages > RSC_MEM_SEVEN_GB_PAGES)))
		heap->rsc_reserve_ion_enable = true;

	printk("[RSC] ion_sys_heap_reserve ion: %lu pool: %u max_page_pool_size: %u "
	       "mode%d totalram_pages: %lu SEVENGpage: %u resmode: %u enable: %d\n",
	       global_zone_page_state(NR_ION), ion_page_pool_nr_pages(),
	       max_page_pool_size, rsc_recovery_survival_mode, totalram_pages,
	       RSC_MEM_SEVEN_GB_PAGES, rsc_reserve_ion_mode,
	       heap->rsc_reserve_ion_enable);

	c1 = local_clock();

	if (!rsc_recovery_survival_mode && rsc_system_heap &&
	    heap->rsc_reserve_ion_enable) {
		cpu = raw_smp_processor_id();
		for (i = 0; i <= max((const unsigned int)1, order_to_index(4));
		     i++) {
			if (global_zone_page_state(NR_ION) <
			    reserve_max_pool_size) {
				pools = rsc_system_heap->cached_pools[i];
				/*
				* max_page_pool_size expend to max_page_pool_size * 13 /10,
				* max_page_pool_size 700MB expend to 700MB * 13 /10,
				*/
				size = set_size[i] << (20 - PAGE_SHIFT);
				charge_count +=
					ion_sys_heap_charge(pools, size);
				printk("[RSC] ion_sys_heap_reserve i:%d size: %d, NR_ION:%d\n",
				       i, size, global_zone_page_state(NR_ION));
			}
		}
		c2 = local_clock();
		printk("[RSC] ion_sys_heap_reserve ion: %lu pool: %u max_page_pool_size: %u "
		       "charge_count: %u cost: %llu us cpu%d %d\n",
		       global_zone_page_state(NR_ION),
		       ion_page_pool_nr_pages(), max_page_pool_size,
		       charge_count, (c2 - c1) / 1000, cpu,
		       raw_smp_processor_id());
	} else {
	out:
		printk("[RSC] ion_sys_heap_reserve ion: %lu pool: %u max_page_pool_size: %u "
		       "skip reserve idx %u mode%d rsc_system_heap: %p totalram_pages: %lu  "
		       "SEVENGpage: %u resmode: %u enable: %d\n",
		       global_zone_page_state(NR_ION),
		       ion_page_pool_nr_pages(), max_page_pool_size,
		       atomic_read(&reserve_time), rsc_recovery_survival_mode,
		       rsc_system_heap, totalram_pages,
		       RSC_MEM_SEVEN_GB_PAGES, rsc_reserve_ion_mode,
		       heap->rsc_reserve_ion_enable);
	}
}

static inline void ion_page_pool_free_pages_now(struct ion_page_pool *pool,
						struct page *page)
{
...
static int ion_page_pool_shrink_fast(struct ion_page_pool *pool, int nr_to_scan)
{
	int freed = 0;
	...
	while (freed < nr_to_scan) {
		...
		if (pool->low_count) {
			page = rsc_ion_page_pool_remove(pool, false);
		} else {
...
static int ion_page_pools_shrink_fast(struct ion_system_heap *sys_heap,
				      int nr_to_scan)
{
	int nr_total = 0;
	int i, nr_freed;
	...
	for (i = num_orders - 1; i >= 0; i--) {
		...
		nr_freed = ion_page_pool_shrink_fast(pool, nr_to_scan);
		nr_to_scan -= nr_freed;
		nr_total += nr_freed;
		/* shrink completed */
		if (nr_to_scan <= 0)
...
static void free_buffer_page_internal(struct ion_system_heap *heap,
				      struct ion_buffer *buffer,
				      struct page *page, unsigned int order)
{
	...
	curordcnt = (pool->low_count << order);
	/* prevent that many pages are not free in page pool list*/
	if (buffer->private_flags & ION_PRIV_FLAG_SHRINKER_FREE) {
		if (orders[0] == order) {
			if (curordcnt > reserve_order_size[0]) {
				ion_page_pool_free_pages_now(pool, page);
				printk("freebuffer orders[0] count: %u max_page_pool_size: %u buffer->size: %lu fullquick! "
				       "order: %d pool: %d curordcnt %d max %d res %d idx%d\n",
				       ion_page_pool_nr_pages(),
				       max_page_pool_size,
				       (unsigned long)buffer->size, order,
				       (pool->low_count << order), curordcnt,
				       curordcnt, reserve_order_size[0],
				       order_to_index(orders[0]));
				return;
			}
		} else if (orders[1] == order) {
			int ord9cnt = (heap->cached_pools[0]->low_count
				       << orders[0]); /*order8 pool count*/

			if ((curordcnt > reserve_order_size[1]) &&
			    (curordcnt + ord9cnt) > (reserve_order_size[0] +
						     reserve_order_size[1])) {
				ion_page_pool_free_pages_now(pool, page);
				printk("freebuffer orders[1] count: %u max_page_pool_size: %u buffer->size: %lu fullquick! "
				       "order: %d pool: %d curordcnt %d max %d res %d idx%d\n",
				       ion_page_pool_nr_pages(),
				       max_page_pool_size,
				       (unsigned long)buffer->size, order,
				       (pool->low_count << order), curordcnt,
				       ord9cnt, reserve_order_size[1],
				       order_to_index(orders[0]));
				return;
			}
		}
		...
		*/
		if (global_zone_page_state(NR_ION) > rsc_system_max_ion_page) {
			ion_page_pools_shrink_fast(heap,
						   (ION_OPT_SHRINK_POOL_SIZE_MB
						    << (20 - PAGE_SHIFT)));
			printk("freebuffer count: %u max_page_pool_size: %u buffer->size: %lu overload shrink! order: %d  %lu > %d\n",
			       ion_page_pool_nr_pages(),
			       max_page_pool_size, (unsigned long)buffer->size,
			       order, global_zone_page_state(NR_ION),
			       rsc_system_max_ion_page);
		}
	}
...
#ifdef CONFIG_RSC_ION_OPT
	if (heap->rsc_reserve_ion_enable && ion_buffer_cached(buffer)) {
		...
		for_each_sg(table->sgl, sg, table->nents, i)
			free_buffer_page_internal(sys_heap, buffer, sg_page(sg),
					 get_order(sg->length));
	} else {
		/* zero the buffer before goto page pool */
		if (!(buffer->private_flags & ION_PRIV_FLAG_SHRINKER_FREE))
...
#ifdef CONFIG_RSC_ION_OPT
static int ion_page_pool_shrink_reserve_pool(struct ion_system_heap *sys_heap,
		gfp_t gfp_mask, int priority, int only_scan)
{
	int curordcnt, reclaimpage, nr_total = 0;
	bool highpress = (priority <= (DEF_PRIORITY - ion_priority[RSC_ION_POOL_LEVEL-2]));
	u32 reservepage;
	struct ion_page_pool *pool;

	/*prevent reserve order9 pool is too bigger.*/
	pool = sys_heap->cached_pools[0];
	curordcnt = (pool->low_count << orders[0]);
	if (highpress)
		reservepage = reserve_order_size[0];
	else
		reservepage = 2 * reserve_order_size[0];

	if (curordcnt > reservepage) {
		reclaimpage = curordcnt - reservepage;
		if (reclaimpage > (ION_OPT_SLOWPATH_SHRINK_POOL_SIZE_MB << (20 - PAGE_SHIFT)))
			reclaimpage = (ION_OPT_SLOWPATH_SHRINK_POOL_SIZE_MB << (20 - PAGE_SHIFT));
		if (!only_scan)
			nr_total += ion_page_pool_shrink(pool, gfp_mask, reclaimpage);
		else
			nr_total += reclaimpage;
	}
	return nr_total;
}
#endif
		...
		if (only_scan) {
			...
			if (nr_to_scan <= 0) {
				if (heap->rsc_reserve_ion_enable)
					nr_total +=
						ion_page_pool_shrink_reserve_pool(
							sys_heap, gfp_mask,
							heap->priority,
							only_scan);
				break;
...
static void ion_buffer_dump(struct ion_buffer *buffer, struct seq_file *s)
{
	...
	if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA_SEC) {
		struct ion_sec_buffer_info *sec_info =
			(struct ion_sec_buffer_info *)buffer->priv_virt;
		pdbg = &sec_info->dbg_info;
		ION_DUMP(s,
			"0x%p %8zu %4u %6d %3d %3d %3d %3d %3lu(%3lu) 0x%x, 0x%x, %5d(%5d) %16s %5d %16s %llu 0x%x 0x%x 0x%x 0x%x %s\n",
			buffer, buffer->size, buffer->heap->id,
			buffer->kmap_cnt,
			atomic_read(&buffer->ref.refcount.refs),
			buffer->handle_count, sec_info->module_id,
			1,
			sec_info->MVA, sec_info->FIXED_MVA,
			sec_info->security,
			buffer->flags, buffer->pid, sec_info->pid,
			buffer->task_comm,
			buffer->tid,
	...
	/* only ion_mm_heap contains ion_mm_buffer_info. let's make it sure */
	if (unlikely((int)buffer->heap->type != ION_HEAP_TYPE_MULTIMEDIA))
		return;
			      ...
			      unsigned int fd)
{
	...
	if ((int)buffer->heap->type == ION_HEAP_TYPE_MULTIMEDIA_SEC) {
		struct ion_sec_buffer_info *sec_bug_info =
			(struct ion_sec_buffer_info *)buffer->priv_virt;
		pid = sec_bug_info->pid;
		mva[0] = sec_bug_info->MVA;
		mva_fix[0] = sec_bug_info->FIXED_MVA;
		block_nr[0] = (buffer->size +
					MVA_ALIGN_MASK) >> MVA_SIZE_ORDER;
		for (i = 1; i < DOMAIN_NUM; i++) {
			mva[i] = 0;
			mva_fix[i] = 0;
			block_nr[i] = 0;
		}
	...
	/* 加上这个条件判断 */
        if (buffer->heap->type != ION_HEAP_TYPE_MULTIMEDIA &&
            buffer->heap->type != ION_HEAP_TYPE_SYSTEM)
                return 0;

	bug_info = (struct ion_mm_buffer_info *)buffer->priv_virt;
	if (bug_info) {
		/* if some heap have buffer_info but not MEDIA, not show it */
		if (unlikely((int)buffer->heap->type != ION_HEAP_TYPE_MULTIMEDIA))
			return 0;
...
static int ion_sys_heap_worker(void *data)
{
	...
	for (;;) {
		for (i = 0; i < num_orders; i++) {
			if (pools[i]->order >= VALUEABLE_MIN_REFILL_ORDER &&
			    pool_count_below_lowmark(pools[i]))
...
#ifdef CONFIG_RSC_ION_REFILL
	heap->kworker[ION_UNCACHE_PAGE_POOL] =
		ion_create_kworker(heap->pools, false);
	if (IS_ERR(heap->kworker[ION_UNCACHE_PAGE_POOL]))
		goto err_create_pool;

	heap->kworker[ION_CACHE_PAGE_POOL] =
		ion_create_kworker(heap->cached_pools, true);
	if (IS_ERR(heap->kworker[ION_CACHE_PAGE_POOL]))
		goto err_create_pool;
#endif

#ifdef CONFIG_RSC_ION_OPT
	if (unused->id == ION_HEAP_TYPE_MULTIMEDIA_FOR_CAMERA) {
		INIT_DELAYED_WORK(&heap->heap.reserve_ion_worker,
				  ion_sys_heap_reserve);
		heap->heap.rsc_reserve_ion_enable = false;
		/*move to pool_size_array_set*/
		schedule_delayed_work(&heap->heap.reserve_ion_worker, 2*HZ);
	}
#endif
