...
/* vivo liuyonghong modify for upgrading PD2083F_EX_12.0 change watermark_scale_factor10 to 1 */
		...
		unsigned long mark, int classzone_idx, unsigned int alloc_flags)
{
	...
	if (!order) {
		...
		if (unmovable_isolate_enabled(z)) {
			if (!rsc_valid_isolate_order(order, MIGRATE_UNMOVABLE_ISOLATE_SORD))
				unused_free_pages +=
						zone_page_state(z, NR_FREE_UNMOVABLE_ISOLATE_SORD);

			if (!((alloc_flags & ALLOC_UNMOVABLE_BORD) &&
				(rsc_valid_isolate_order(order, MIGRATE_UNMOVABLE_ISOLATE_BORD))))
				unused_free_pages +=
						zone_page_state(z, NR_FREE_UNMOVABLE_ISOLATE_BORD);
		}
#endif
...
/*
 * if costly order alloc failed check the
 * file pages is over the limit(1GB) will do drop cache with sync mode
 */
#define FILE_PAGES_LIMIT	262144
...
retry:
	...
	if (costly_order && !(gfp_mask & __GFP_RETRY_MAYFAIL)) {
		...
		if (!have_do_drop_filecache && ((unsigned long) uptime.tv_sec < 100) &&
			(global_node_page_state(NR_LRU_BASE + LRU_FILE) > FILE_PAGES_LIMIT)) {
