...
static const struct snd_kcontrol_new mt6359_snd_controls[] = {
	/* dl pga gain */
	/* vivo audio dxl add headset ctrl to compatible ard11 start */
	...
	SOC_SINGLE_EXT_TLV("LineoutR Volume",
			   ...
			   snd_soc_get_volsw, mt6359_put_volsw, playback_tlv),
	/* vivo audio dxl add headset ctrl to compatible ard11 end */
			       ...
			       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_update_bits(priv->regmap, MT6359_AUDENC_ANA_CON15,
				   ...
				   MIC_BIAS_2P1 << RG_AUDMICBIAS0VREF_SFT); // modify by xudong for MISBIAS0 =2.1v
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (mic_type == MIC_TYPE_MUX_DCC_ECM_SINGLE)
			regmap_write(priv->regmap,
				     MT6359_AUDENC_ANA_CON16, 0x0170); // modify by xudong for MISBIAS =2.7v
		else
			regmap_write(priv->regmap,
				     MT6359_AUDENC_ANA_CON16, 0x0070); // modify by xudong for MISBIAS =2.7v
			       ...
			       int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		regmap_update_bits(priv->regmap, MT6359_AUDENC_ANA_CON17,
				   ...
				   MIC_BIAS_2P1 << RG_AUDMICBIAS2VREF_SFT); // modify by xudong for MISBIAS2 =2.1v
		/* vow low power select */
		regmap_update_bits(priv->regmap, MT6359_AUDENC_ANA_CON17,
...
/* ADD by xudong, for bottom noise when fast charging */
/* Audio module needs to provide an flag of checking whether headset mic is using to Charge module */
bool voip_switch_flag;
bool mic_switch_flag;
unsigned int fast_charge_flag; /* Add by vivo audio xudong for compatible fast charge */
/* end */

void change_voip_status(bool flag)
{
	voip_switch_flag = flag;
}
EXPORT_SYMBOL(change_voip_status);

void changeFSM()
{
	printk("%s, mic record status %d, VOIP status %d, fast_charge_flag %d\n", \
			__func__, mic_switch_flag, voip_switch_flag, fast_charge_flag);

	/* Add by vivo audio xudong for compatible fast charge */
	if(fast_charge_flag == 0)
		return;

}
EXPORT_SYMBOL(changeFSM);
			  ...
			  int event)
{
	...
	unsigned int mic_type_index; /* Add by vivo audio xudong for compatible fast charge */
	...
	/* Add by vivo audio xudong for compatible fast charge start */
	if (fast_charge_flag == 1)
		mic_type_index = MUX_MIC_TYPE_0;
	else if (fast_charge_flag == 2)
		mic_type_index = MUX_MIC_TYPE_1;
	else
		mic_type_index = MUX_MIC_TYPE_1;
	/* Add by vivo audio xudong for compatible fast charge end*/

	/* if is VOW, then force 24dB */
	if (priv->vow_enable)
		mic_gain_l = 4;
	dev_info(priv->dev, "%s(), event = 0x%x, mic_type %d, mic_gain_l %d, mux_pga %d, mic_type index %d\n",
		 __func__, event, mic_type, mic_gain_l, mux_pga, mic_type_index);

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		/* Add by vivo audio xudong for compatible fast charge start */
		if ((mic_type == priv->mux_select[mic_type_index]) && fast_charge_flag) {
			mic_switch_flag = true;
			changeFSM();
		}
		/* Add by vivo audio xudong for compatible fast charge end */
...
static int mt6359_codec_init_reg(struct mt6359_priv *priv)
{
	...
	int miso_drv = 0, res = 0;
	res = of_property_read_u32(priv->dev->of_node, "mt6359-miso-driving", &miso_drv);
	if (!res)
		dev_info(priv->dev, "%s(), miso_drv: %d\n", __func__, miso_drv);
	...
	/********************* wangkairjptb add for audio begin *********************/
	/* gpio miso driving set */
	if (miso_drv == 8) {
		dev_info(priv->dev, "%s(), set miso_drv: %dmA\n",__func__, miso_drv);
		regmap_write(priv->regmap, MT6359_DRV_CON3, 0xaaaa);
		regmap_write(priv->regmap, MT6359_DRV_CON1, 0xaaaa);
	} else if (miso_drv == 12) {
		dev_info(priv->dev, "%s(), set miso_drv: %dmA\n",__func__, miso_drv);
		regmap_write(priv->regmap, MT6359_DRV_CON3, 0xcccc);
		regmap_write(priv->regmap, MT6359_DRV_CON1, 0xcccc);
	} else if (miso_drv == 16) {
		dev_info(priv->dev, "%s(), set miso_drv: %dmA\n",__func__, miso_drv);
		regmap_write(priv->regmap, MT6359_DRV_CON3, 0xeeee);
		regmap_write(priv->regmap, MT6359_DRV_CON1, 0xeeee);
	} else {
		dev_info(priv->dev, "%s(), set default 4mA\n",__func__);
		regmap_write(priv->regmap, MT6359_DRV_CON3, 0x8888);
		regmap_write(priv->regmap, MT6359_DRV_CON1, 0x8888);
	}
	/********************* wangkairjptb add for audio end *********************/
...
static void mt6359_parse_dt(struct mt6359_priv *priv)
{
	...
	/* Add by vivo audio xudong for compatible fast charge start*/
	ret = of_property_read_u32(dev->of_node, "vivo,audio-forbid-fast-charge", &fast_charge_flag);
	if (ret) {
		dev_info(dev, "%s() failed to read audio-forbid-fast-charge, default 0\n",
			 __func__);
		fast_charge_flag = 0;
	}
	/* Add by vivo audio xudong for compatible fast charge end*/
