			       ...
			       struct ipi_msg_t *ipi_msg, int id)
{
	...
	if (ipi_msg && ipi_msg->param2 == ADSP_DL_CONSUME_UNDERFLOW) {
		pr_info("%s adsp adsp_xrun_flag id = %d\n", __func__, id);
		dsp->dsp_mem[id].adsp_xrun_flag = true;
		snd_pcm_period_elapsed(dsp->dsp_mem[id].substream);
		return true;
	}
...
static int mtk_dsp_pcm_open(struct snd_pcm_substream *substream)
{
	...
	/******************** vivo audio team add begin ********************/
	if (dsp->dsp_mem[id].adsp_xrun_flag) {
		pr_info("%s id = %d adsp_xrun_flag[%d], reset\n",
			__func__, id, dsp->dsp_mem[id].adsp_xrun_flag);
		dsp->dsp_mem[id].adsp_xrun_flag = false;
	}
	/******************** vivo audio team add end ********************/
