...
struct mtk_base_afe *afe_base; //xudong add for i2s setting
...
extern void changeFSM(void);
extern void change_voip_status(bool flag);
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (rtd->cpu_dai->id == MT6833_MEMIF_DL12) {
		change_voip_status(true);
		changeFSM();
	}
...
static int mt6833_afe_pcm_dev_probe(struct platform_device *pdev)
{
	...
	afe_base = afe; //xudong add for i2s seting
