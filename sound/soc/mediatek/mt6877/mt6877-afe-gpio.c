...
static struct audio_gpio_attr aud_gpios[MT6877_AFE_GPIO_GPIO_NUM] = {
	[MT6877_AFE_GPIO_CLK_MOSI_OFF] = {"aud_clk_mosi_off", false, NULL},
	[MT6877_AFE_GPIO_CLK_MOSI_ON] = {"aud_clk_mosi_on", false, NULL},
...
static int mt6877_afe_gpio_adda_clk(struct mtk_base_afe *afe, bool enable)
{
	static int counter;
	if (enable) {
		counter++;
		mt6877_afe_gpio_select(afe, MT6877_AFE_GPIO_CLK_MOSI_ON);
	} else {
		counter--;
		if (counter <= 0) {
			counter = 0;
			mt6877_afe_gpio_select(afe, MT6877_AFE_GPIO_CLK_MOSI_OFF);
		}
	}
	return 0;
}
			    ...
			    int dai, int uplink)
{
	...
	case MT6877_DAI_ADDA:
		mt6877_afe_gpio_adda_clk(afe, enable);
		if (uplink) {
