...
static int mt6877_mt6359_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < card->num_links; i++) {
		...
		//yuchun.yang add for vivo codec begin
		if (!strcmp(mt6877_mt6359_dai_links[i].name, "VIVOCODECPLAYBACK") && vivo_codec_name) {
			dev_info(&pdev->dev, "%s:vivo_codec_name %s\n", __func__, vivo_codec_name);
			mt6877_mt6359_dai_links[i].codec_name = vivo_codec_name;
			mt6877_mt6359_dai_links[i].codec_dai_name = "VIVO-Codec-dai";
			continue;
		}
		//yuchun.yang add end
