...
// wangkai add
static const char *const mtk_scp_ultra_AT_str[] = {
	"Normal",
	"AT_mode"};
...
/* wangkai add */
struct ultra_ATmode_nearfar{
	unsigned int refV;
	unsigned int staticV;
};
static int ATmode;
static bool AT_near_far_flag;
static struct ultra_ATmode_nearfar nf = {
	.refV = 0,
	.staticV = 0,
};

static int using_primary_mic; // default is 0, hands-free is 1
/* end */
...
{
	unsigned int temp = *((unsigned int *)msg_data); /* wangkai add */
	switch (msg_id) {
	case AUDIO_TASK_USND_MSG_ID_AT_NEARFAR_OK: { /* wangkai add */
		printk("%s ultrasound ref val = %u, static val = %u\n", __func__, temp>>16, temp&0xffff);
		nf.staticV = (temp&0xffff);
		nf.refV = (temp>>16);
		AT_near_far_flag = true;
		break;
	}
...
static const struct soc_enum mtk_scp_ultra_enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mtk_scp_ultra_AT_str), /* wangkai add */
			    mtk_scp_ultra_AT_str)
};
...
/* wangkairjptb add for audio begin */
// wangkai add for AT MODE
static int mtk_scp_ultra_ATmode_config_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	if (ATmode > 1 && ATmode < 0){
		printk("[ultrasound] %s, get ATmode from kernel meets error (value is %d)\n", __func__, ATmode);
		return -EINVAL;
	}
	ucontrol->value.integer.value[0] = ATmode;
	printk("[ultrasound] %s, get ATmode from kernel successful (value is %d)\n", __func__, ATmode);
	return 0;
}

static int mtk_scp_ultra_ATmode_config_set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	int payload;

	if (ucontrol->value.integer.value[0] !=0 && ucontrol->value.integer.value[0] !=1) {
		printk("[ultrasound] %s, send ATmode to SCP meets error (value is %d)\n", __func__, ucontrol->value.integer.value[0]);
		return -EINVAL;
	}

	ATmode = ucontrol->value.integer.value[0];
	if (ATmode == 1) {
		payload = 2;
		printk("[ultrasound] %s, send ATmode to SCP, enter AT mode\n", __func__);
			 ultra_ipi_send(AUDIO_TASK_USND_MSG_ID_ATMODE,
			 false,
			 1,
			 &payload,
			 ULTRA_IPI_BYPASS_ACK);
		...
		payload = 0;
		printk("[ultrasound] %s, send ATmode to SCP, enter Normal\n", __func__);
			 ultra_ipi_send(AUDIO_TASK_USND_MSG_ID_NO_ATMODE,
			 false,
			 1,
			 &payload,
			 ULTRA_IPI_BYPASS_ACK);
...
/*
	// wangkai add
	printk("%s, ultrasound mtk_ipi_recv IPI_OUT_AUDIO_ULTRA_SND_ACK_0", __func__);
	ipi_ack_return = mtk_ipi_recv(&scp_ipidev, IPI_OUT_AUDIO_ULTRA_SND_ACK_0);
	printk("%s, ultrasound mtk_ipi_recv ipi_ack_return=%d, ipi_ack_id=%d\n", __func__, ipi_ack_return, ipi_ack_id);
	// end
*/

static int mtk_scp_ultra_Atmode_nearfar_get(struct snd_kcontrol *kcontrol,
					 unsigned int __user *data,
					 unsigned int size)
{
	struct ultra_ATmode_nearfar nearfar = {0, 0};
	bool ret;
	int cnt = 5;

start:
	while (cnt--){
		ret = ultra_ipi_send(AUDIO_TASK_USND_MSG_ID_AT_NEARFAR, false, 0, NULL, ULTRA_IPI_BYPASS_ACK);
		if (!ret) {
			printk("%s, ultrasound ultra_ipi_send failed\n", __func__);
			mdelay(30);
			goto start;
		}
		else {
			printk("%s, ultrasound ultra_ipi_send successful\n", __func__);
			break;
		}
	}
	...
	mdelay(100);
	if (AT_near_far_flag) {
		if (copy_to_user(data, &nf, sizeof(struct ultra_ATmode_nearfar))) {
			printk("%s, ultrasound send data to HAL failed\n", __func__);
			return -EFAULT;
		}
		printk("%s, ultrasound send data to HAL successful (ref = %u, static = %u)\n", __func__, nf.refV, nf.staticV);
		AT_near_far_flag = false;
		return 0;
	}

	if (copy_to_user(data, &nearfar, sizeof(struct ultra_ATmode_nearfar))) {
		printk("%s, ultrasound send data to HAL failed\n", __func__);
		return -EFAULT;
	}
	printk("%s, ultrasound can not send data to HAL\n", __func__);

	return 0;
}

static int mtk_scp_ultra_Atmode_nearfar_set(struct snd_kcontrol *kcontrol,
					 const unsigned int __user *data,
					 unsigned int size)
{
	struct ultra_ATmode_nearfar nearfar;
	if (copy_from_user(&nearfar, data, sizeof(struct ultra_ATmode_nearfar))) {
		printk("%s, ultrasound recv data from HAL failed\n", __func__);
		return -EFAULT;
	}
	printk("[ultrasound] %s, recv data from HAL: ref = %u, static = %u\n", __func__, nearfar.refV, nearfar.staticV);
	printk("[ultrasound] %s, no need to set ref val and static val\n", __func__);
	return 0;
}

static int mtk_scp_ultra_infrared_value_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = 1;
	printk("[ultrasound] %s, ultrasound no need to get infrared value from SCP\n", __func__);
	return 0;
}

static int mtk_scp_ultra_infrared_value_set(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	int payload;

	payload = ucontrol->value.integer.value[0];

	printk("[ultrasound] %s, send infrared_value to SCP (%d)\n", __func__, payload);
		 ultra_ipi_send(AUDIO_TASK_USND_MSG_ID_INFRARED,
		 false,
		 1,
		 &payload,
		 ULTRA_IPI_BYPASS_ACK);
...
static int vivo_scp_ultra_using_mic_get(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	if (using_primary_mic > 1 || using_primary_mic < 0){
		printk("[ultrasound] %s, get using_primary_mic from kernel meets error (value is %d)\n", __func__, using_primary_mic);
		return -EINVAL;
	}
	ucontrol->value.integer.value[0] = using_primary_mic;
	printk("[ultrasound] %s, get using_primary_mic from kernel successful (value is %d)\n", __func__, using_primary_mic);
	return 0;
}

static int vivo_scp_ultra_using_mic_set(struct snd_kcontrol *kcontrol,
					  struct snd_ctl_elem_value *ucontrol)
{
	int payload = 0;
	using_primary_mic = ucontrol->value.integer.value[0];
	if (using_primary_mic < 0 || using_primary_mic > 1) {
		printk("[ultrasound] %s, using_primary_mic = %d invalid, using default 0\n", __func__, using_primary_mic);
		using_primary_mic = 0;
	}

	payload = using_primary_mic;
	printk("[ultrasound] %s, send using_primary_mic = %d to SCP\n", __func__, payload);
		 ultra_ipi_send(AUDIO_TASK_USND_MSG_ID_MIC_CHOOSE,
		 false,
		 1,
		 &payload,
		 ULTRA_IPI_BYPASS_ACK);
...
/* wangkairjptb add for audio end */
...
/* wangkairjptb add for audio begin */
	SOC_ENUM_EXT("mtk_scp_ultra_ATmode_config", mtk_scp_ultra_enum[1],
			  mtk_scp_ultra_ATmode_config_get,
			  mtk_scp_ultra_ATmode_config_set),
	SND_SOC_BYTES_TLV("mtk_scp_ultra_ATmode_nearfar",
			  sizeof(struct ultra_ATmode_nearfar),
			  mtk_scp_ultra_Atmode_nearfar_get,
			  mtk_scp_ultra_Atmode_nearfar_set),
	SOC_SINGLE_EXT("mtk_scp_ultra_Infrared_value", SND_SOC_NOPM, 0, 0xffffffff, 0,
	       mtk_scp_ultra_infrared_value_get, mtk_scp_ultra_infrared_value_set),
	SOC_SINGLE_EXT("vivo_scp_ultra_using_mic",
			   SND_SOC_NOPM, 0, 0xff, 0,
			 vivo_scp_ultra_using_mic_get,
			 vivo_scp_ultra_using_mic_set),
/* wangkairjptb add for audio end */
};
