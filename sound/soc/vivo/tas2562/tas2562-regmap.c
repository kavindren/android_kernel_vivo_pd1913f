	...
	unsigned int reg, unsigned int value)
{
	...
	if (chn&channel_left) {
		...
		if (n_result < 0)
			...
			dev_dbg(p_tas2562->dev,
			"%s: chn%x:BOOK:PAGE:REG 0x%02x:0x%02x:0x%02x, VAL: 0x%02x\n",
	...
	if ((chn&channel_right) && (p_tas2562->mn_channels == 2)) {
		...
		if (n_result < 0)
			...
			dev_dbg(p_tas2562->dev, "%s: chn%x:BOOK:PAGE:REG 0x%02x:0x%02x:0x%02x, VAL: 0x%02x\n",
				__func__, p_tas2562->client->addr,
				TAS2562_BOOK_ID(reg),
				TAS2562_PAGE_ID(reg),
				TAS2562_PAGE_REG(reg), value);
	...
	unsigned int reg, unsigned char *p_data, unsigned int n_length)
{
	...
	if (chn&channel_left) {
		...
		if (n_result < 0)
			...
			dev_dbg(p_tas2562->dev, "%s: chn%x:BOOK:PAGE:REG 0x%02x:0x%02x:0x%02x, len: 0x%02x\n",
				__func__, p_tas2562->client->addr,
				TAS2562_BOOK_ID(reg), TAS2562_PAGE_ID(reg),
	...
	if ((chn&channel_right) && (p_tas2562->mn_channels == 2)) {
		...
		if (n_result < 0)
			...
			dev_dbg(p_tas2562->dev, "%s: %x:BOOK:PAGE:REG 0x%02x:0x%02x:0x%02x, len: 0x%02x\n",
				__func__, p_tas2562->client->addr,
				TAS2562_BOOK_ID(reg), TAS2562_PAGE_ID(reg),
				TAS2562_PAGE_REG(reg), n_length);
	}
...
static void irq_work_routine(struct work_struct *work)
{
	...
	dev_info(p_tas2562->dev, "%s, chn=%d channels=%d\n", __func__, chn, p_tas2562->mn_channels);
	...
	if ((chn & channel_right) && (n_result >= 0))
		n_result = p_tas2562->read(p_tas2562, channel_right,
