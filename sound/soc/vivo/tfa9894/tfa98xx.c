...
/* added by wangkairjptb */
struct tfa98xx *tfa98xx_priv;
...
static int tfa98xx_debug_cali(int force)
{
	int ret = 0;
	tfa98xx_mi2s_clk_enable();
	ret = tfa98xx_boot_init(force);
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	nr_controls = 4; /* Profile and stop control and mute control and TFA_CHIP_SELECTOR control */
...
/* debug node, added by wangkairjptb */
...
static ssize_t smartpa_otc_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	...
	if (tfa98xx == NULL) {
		pr_err("%s : smartpa tfa98xx is NULL!\n", __func__);
		return -1;
	}
			 ...
			 const char *ubuf, size_t count)
{
   ...
   mutex_lock(&tfa98xx->dsp_lock);
   tfa98xx_set_exttemp_v1(tfa98xx->tfa, kbuf);
   mutex_unlock(&tfa98xx->dsp_lock);
		...
		const char *ubuf, size_t count)
{
    ...
    mutex_lock(&tfa98xx->dsp_lock);
    ret = tfa_calibrate_v1(tfa98xx->tfa);
...
static ssize_t smartpa_rpc_show(struct kobject *kobj, struct kobj_attribute *attr, char *ubuf)
{
	...
	if (tfa98xx->tfa == NULL || tfa98xx->i2c == NULL) {
		pr_err("%s, dsp or i2c is not available\n", __func__);
		return -ENODEV;
	}
...
static ssize_t smartpa_fw_debug_show(struct kobject *kobj, struct kobj_attribute *attr, char *buffer)
{
	const int size = 512;
	int n = 0;
	const char *str = "Note: test for debug fw or reset for default fw\n"   \
						"step1) 'push tfa98xx.cnt to /vendor/firmware/tfa98xx_debug.cnt'\n" \
						"step2) 'echo test > fw_debug' or 'echo reset > fw_debug'\n\n";
...
static ssize_t smartpa_fw_debug_store(struct kobject *kobj, struct kobj_attribute *attr,
				const char *ubuf, size_t count)
{
	struct tfa98xx *tfa98xx = tfa98xx_priv;
	nxpTfaContainer_t *old_tfa98xx_container = NULL;
	...
	/* Compare strings, excluding the trailing \0 */
	if (!strncmp(ubuf, test_cmd, sizeof(test_cmd) - 1)) {
		fw_name = "tfa98xx_debug.cnt";
	} else if (!strncmp(ubuf, reset_cmd, sizeof(reset_cmd) - 1)) {
		fw_name = default_fw_name;
	} else {
		pr_err("[SmartPA-%d]: enter %s err\n", __LINE__, __func__);
		return -EINVAL;
	}
...
static struct kobj_attribute dev_attr_fw_debug =
	__ATTR(fw_debug, 0664, smartpa_fw_debug_show, smartpa_fw_debug_store);
...
static int tfa98xx_probe (struct snd_soc_codec *codec)
{
	...
	class_attr_create(tfa98xx->kobj);
